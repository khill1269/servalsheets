...
### STATUS UPDATE — 2026-02-28 (Current Branch Reality)

Re-verified against current branch after Phase 0 integrity fixes:

- `npm run typecheck` ✅ (0 errors)
- `npx vitest run tests/handlers/federation.test.ts tests/handlers/format.test.ts` ✅
- `npm run validate:action-config` ✅
- `npm run check:drift` ✅ (25 tools / 391 actions)
- `npm run validate:alignment` ✅
- `npm run validate:actions` ✅ (run outside sandbox due tsx IPC restriction)

Known caveat:

- `npm run test:fast` is green. Current run reports `103 passed | 1 skipped` files and `2391 passed | 37 skipped` tests.
- `tests/core/redis-task-store.test.ts` is skip-gated when Redis is not reachable, so Redis-dependent tests are skipped instead of hanging.

### STATUS UPDATE — 2026-03-03 (Session 53 Continuation)

Focused P2 hardening updates verified:

- Direct sampling consent guards were added for remaining decomposed analyze/format/service call paths:
  - `src/handlers/analyze-actions/query-natural-language.ts`
  - `src/handlers/analyze-actions/explain.ts`
  - `src/handlers/format-actions/validation.ts`
  - `src/services/llm-fallback.ts`
  - `src/services/sheet-generator.ts`
- `sheets_core.batch_get` now emits MCP progress notifications for large request sets (`src/handlers/core-actions/spreadsheet-read.ts`, wired from `src/handlers/core.ts`).
- Helper-level sampling consent coverage extended:
  - `src/mcp/sampling.ts`: `analyzeDataStreaming` summary call + `streamAgenticOperation` now require consent.
  - `src/services/agent-engine.ts`: defaults to global sampling consent guard when agent-local checker is unset.
- Progress coverage phase-2 extended:
  - `src/handlers/analyze-actions/formulas.ts`: progress notifications for multi-sheet formula scans.
  - `src/handlers/advanced-actions/chips.ts`: progress notifications for multi-sheet chip scans.
  - Wiring added in `src/handlers/analyze.ts` and `src/handlers/advanced.ts`.
  - `src/handlers/composite-actions/batch.ts`: progress notifications for multi-operation `batch_operations` loops.
  - Wiring added in `src/handlers/composite.ts`.
  - `src/handlers/dependencies.ts`: progress notifications for multi-change `model_scenario` and multi-scenario `compare_scenarios` analyses.
  - `src/handlers/templates.ts`: progress notifications for multi-sheet `apply` template operations.
- Regression coverage added:
  - `tests/unit/llm-fallback-consent.test.ts` (new)
  - `tests/handlers/core.test.ts` progress-notification assertion for `batch_get`
  - `tests/mcp/sampling-agentic-consent.test.ts` (new)
  - `tests/services/agent-engine-consent.test.ts` (new)
  - `tests/handlers/analyze.test.ts` progress-notification assertion for `analyze_formulas`
  - `tests/handlers/advanced.test.ts` progress-notification assertion for `list_chips`
  - `tests/handlers/composite.test.ts` progress-notification assertion for `batch_operations`
  - `tests/handlers/dependencies.test.ts` progress-notification assertions for `model_scenario` and `compare_scenarios`
  - `tests/handlers/templates.test.ts` progress-notification assertion for multi-sheet `apply`
- Verification snapshot:
  - `npm run typecheck` ✅
  - targeted `eslint` (with `NODE_OPTIONS=--max-old-space-size=8192`) ✅
  - `npx vitest run tests/mcp/sampling-consent-cache.test.ts tests/mcp/sampling-agentic-consent.test.ts tests/services/agent-engine-consent.test.ts tests/unit/llm-fallback-consent.test.ts tests/handlers/core.test.ts tests/handlers/analyze.test.ts tests/handlers/advanced.test.ts tests/handlers/composite.test.ts tests/handlers/dependencies.test.ts tests/handlers/templates.test.ts` ✅ (`272/272`)
...
_These 25 issues were identified during a cross-reference pass of all 8 source documents.
They were not captured in the initial 88-issue extraction (Sections 1–9 above)._
...
_Found by re-reading all 8 source documents exhaustively via parallel agents:
Handler_Analysis §1-42, Implementation_Guide T1-T12, Fix_Report all sheets,
MCP_Test_Results all sheets, Audit_Report, Competitive_Analysis, Complete_Audit_Roadmap, Strategic_Roadmap._
...
Several handlers use `A1:ZZ10000` as the fallback range when no range is specified (702 columns × 10,000 rows = 7.02M cells). NC-12 prohibits unbounded _column_ refs like `A:Z`, but this bounded-but-enormous default is not covered by NC-12 and passes schema validation.
...
Sampling/Tasks/Elicitation silently fail or hang when the MCP client doesn't support them. No probe of client capabilities occurs before use. ISSUE-107 addresses sending deprecation notices _from_ server _to_ client; this is about checking _client_ capabilities _before_ calling advanced features.
...
Previously-rejected suggestions are filtered _after_ running all 5 pattern detectors. For large sheets, full analysis runs even if 90% of suggestions would be excluded by session filters.
...
`ENABLE_RBAC=OFF` by default. Without RBAC, any authenticated token has access to all 340 actions including destructive operations (`delete_sheet`, `revert_to`, `clear`). No one-command RBAC setup path exists. ISSUE-072 covers per-action _OAuth scope_ checks; RBAC is an application-layer access control system.
...
**Severity**: MEDIUM | **Source**: Handler*Analysis §26.2, Implementation_Guide T10 §11.4
**Files**: `src/handlers/webhooks.ts`, `src/handlers/federation.ts`
**Related**: ISSUE-057 (validates \_incoming* trace IDs — this is _outgoing_ propagation)
...
Art. 15 requires that users can request all data held about them, with a formal structured response. `exportProfile()` (ISSUE-090) addresses Art. 20 (portability). Art. 15 requires an additional formal "data inventory" response listing _all_ data categories held and why.
...
**Fix**: Add `dataSensitivity: 'PUBLIC' | 'INTERNAL' | 'SENSITIVE' | 'PII'` to audit log entries. Tag operations that access cell _values_ as `PII` (potentially). Tag structure/metadata operations as `PUBLIC`. Apply separate retention periods by sensitivity level.
...
The majority of tests cover success scenarios. Error conditions (invalid `spreadsheetId`, `insufficientPermissions`, rate limit, circuit breaker open, BigQuery quota exceeded) are untested in most handlers. ISSUE-076/077/078 cover _total test ratios_ for specific tools; this tracks the systematic error-path gap.
...
The `as unknown as never` exhaustiveness check is placed after the switch statement body, not as the `default:` case. TypeScript only enforces the never type narrowing when the check is _inside_ the switch default.
...
## STATUS UPDATE — 2026-03-03 (Readiness Verification Pass)

Re-verified against current branch using:

- `npm run validate:alignment` ✅
- `npm run validate:compliance` ✅
- `npm run validate:mcp-protocol` ✅
- `npm run test:fast` ✅ (2444 total tests: 2407 pass, 37 skipped)
- `npm run check:drift` ✅ (25 tools / 391 actions)
- `npm run audit:coverage` ❌ (11 failing fixture/schema validations)

Key deltas from the previous snapshot:

- Some previously reported risks are now stale in docs (per-user rate limiting, transaction WAL availability, webhook description quality).
- New pre-phase blocker identified: coverage fixture drift (`audit:coverage` red).
- Mutation safety still needs centralization (formula-sanitization breadth + per-spreadsheet write-lock enforcement in shared pipeline).

---

## STATUS UPDATE — 2026-03-03 (Readiness Remediation Continuation)

Follow-up remediation run results:

- `npm run audit:coverage` ✅ (`1207/1207`)
- `npx vitest run tests/mcp/response-builder.test.ts` ✅
- `npx vitest run tests/handlers/composite.test.ts tests/handlers/base.test.ts tests/handlers/optimization.test.ts` ✅

Key deltas from the earlier 2026-03-03 snapshot:

- `ISSUE-240` moved to `[FIXED]` (fixture/schema coverage drift resolved).
- `ISSUE-241` moved to `[FIXED]` for known non-canonical runtime emission paths (`RATE_LIMIT` and composite catch-path code).
- Remaining pre-next-phase blockers are mutation-safety centralization (`ISSUE-242`, `ISSUE-243`) and webhook durability decision (`ISSUE-246`).

---

## STATUS UPDATE — 2026-03-03 (Mutation Safety Hardening Pass)

Follow-up remediation run results:

- `npx vitest run tests/middleware/write-lock-middleware.test.ts tests/middleware/mutation-safety-middleware.test.ts` ✅
- `npm run test:fast` ✅ (`2407 passed`, `37 skipped`)
- `npm run typecheck` ✅
- `npm run check:drift` ✅
- `npm run validate:alignment` ✅ (`25/25` tools)
- `npm run validate:mcp-protocol` ✅
- `npm run audit:coverage` ✅ (`1207/1207`)

Key deltas from the prior continuation:

- `ISSUE-242` moved to `[FIXED]` after shared mutation-safety middleware integration.
- `ISSUE-243` moved to `[FIXED]` after expanded multi-spreadsheet write-lock orchestration + concurrency tests.
- Remaining high-priority pre-next-phase item is webhook durability architecture decision (`ISSUE-246`).

---

## STATUS UPDATE — 2026-03-03 (Webhook Durability Decision Pass)

Follow-up remediation run results:

- `npx vitest run tests/handlers/webhooks.test.ts` ✅
- `npm run test:fast` ✅ (`2407 passed`, `37 skipped`)
- `npm run typecheck` ✅

Key deltas from the prior continuation:

- `ISSUE-246` moved to `[FIXED]` with explicit architecture decision: Redis hard requirement (`WEBHOOK_DURABILITY_MODE=redis_required`).
- Remaining tracked pre-next-phase technical cleanup was `ISSUE-245` plus final P19-03 compatibility-policy housekeeping; both are now closed in the readiness closure pass below.

---

## STATUS UPDATE — 2026-03-03 (Readiness Closure Pass)

Follow-up verification run results:

- `npx vitest run tests/contracts/schema-handler-alignment.test.ts` ✅ (`30/30`)
- `npx vitest run tests/mcp/tool-handlers-validation.test.ts` ✅ (`15/15`)
- `npx vitest run tests/utils/error-code-compat.test.ts` ✅ (`8/8`)

Key deltas from the prior continuation:

- `ISSUE-245` moved to `[FIXED]`: contract alignment suite already covers all 25 tools (`agent`, `compute`, `connectors` included) and is green.
- P19-03 compatibility-policy housekeeping is now fully closed in tracking docs (`TASKS.md`, `UNIFIED_IMPROVEMENT_PLAN.md`, `MCP_AUDIT_REPORT.md`, `ServalSheets_Production_Audit.md` sync).
...
## SECTION 15 — PRE-NEXT-PHASE READINESS FINDINGS (2026-03-03)

Issues discovered in the full production-audit verification pass before starting the next implementation phase.

---

### ISSUE-240: [FIXED] HIGH — `audit:coverage` suite was red (11 failing fixtures)

**Severity**: HIGH | **Source**: Pre-phase verification run (2026-03-03)  
**File**: `tests/audit/action-coverage.test.ts` + `tests/audit/action-coverage-fixtures.ts`

`npm run audit:coverage` previously failed due to stale "valid" fixtures that no longer satisfy current schemas:

- `sheets_compute`: `sql_query`, `sql_join`, `python_eval`, `pandas_profile`, `sklearn_model`, `matplotlib_chart`
- `sheets_session`: `schedule_create`, `schedule_cancel`, `schedule_run_now`
- `sheets_webhook`: `subscribe_workspace`, `unsubscribe_workspace`

**Impact**: Coverage audit signal is unreliable; pre-release quality gate is weakened.

**Fix**: Update fixtures to match required schema fields and rerun `npm run audit:coverage`.

**Verification (2026-03-03)**:

- Updated `tests/audit/action-coverage-fixtures.ts` for compute/session/webhook actions.
- `npm run audit:coverage` now passes (`1207/1207`).

---

### ISSUE-241: [FIXED] HIGH — Runtime emitted non-canonical error codes outside `ErrorCodeSchema`

**Severity**: HIGH | **Source**: Pre-phase verification run (2026-03-03)  
**Files**:

- `src/handlers/composite.ts` (now emits `INTERNAL_ERROR` for batch-operation catch path)
- `src/mcp/response-builder.ts` (now canonicalizes `RATE_LIMIT` input to `RATE_LIMITED`)
- `src/schemas/shared.ts` (canonical enum includes `RATE_LIMITED`, not `RATE_LIMIT`)

**Impact**: Client-side error handling drift, weaker contract guarantees, and larger compatibility surface.

**Fix**: Canonicalize all emitted runtime codes to enum-backed values; keep aliases only in compatibility metadata layer.

**Verification (2026-03-03)**:

- Focused tests pass: `tests/mcp/response-builder.test.ts`, `tests/handlers/composite.test.ts`, `tests/handlers/base.test.ts`, `tests/handlers/optimization.test.ts`.
- Runtime source grep no longer shows emitted `code: 'RATE_LIMIT'` or `code: 'OPERATION_FAILED'` paths.

---

### ISSUE-242: [FIXED] CRITICAL — Formula sanitization coverage was partial, not centralized

**Severity**: CRITICAL | **Source**: Pre-phase verification run (2026-03-03)  
**Files**:

- `src/handlers/data-actions/helpers.ts` (existing formula blocklist checks)
- `src/handlers/data-actions/read-write.ts` (checks only on selected write paths)

Sanitization previously existed for specific handler paths but was not enforced as a shared mutation policy across all mutation entry points.

**Impact**: Inconsistent security posture; dangerous formula payloads may still enter via uncovered paths.

**Fix**: Add centralized mutation-safety policy enforcement in shared request pipeline and cover all mutation actions via tests.

**Verification (2026-03-03)**:

- Added shared guard: `src/middleware/mutation-safety-middleware.ts`.
- Wired into shared tool execution path: `src/mcp/registration/tool-handlers.ts` (pre-handler mutation safety check).
- Added test suite: `tests/middleware/mutation-safety-middleware.test.ts` (nested composite + opt-out coverage).

---

### ISSUE-243: [FIXED] CRITICAL — Per-spreadsheet mutation lock was not fully enforced in shared write pipeline

**Severity**: CRITICAL | **Source**: Pre-phase verification run (2026-03-03)  
**Files**:

- `src/services/conflict-detector.ts` (lock primitives exist)
- request pipeline files (`src/mcp/registration/tool-handlers.ts`, relevant middleware/wiring)

Conflict/locking primitives existed, but write-lock extraction and mutation classification were too narrow (`request.spreadsheetId` + limited action set), leaving gaps for nested/multi-spreadsheet mutation payloads.

**Impact**: Concurrent writer sessions can still interleave logical mutations.

**Fix**: Add shared write-lock orchestration keyed by spreadsheet ID and integrate with mutation dispatch path.

**Verification (2026-03-03)**:

- Hardened lock middleware: `src/middleware/write-lock-middleware.ts`
  - extracts nested spreadsheet IDs (`source/destination/spreadsheetIds`)
  - acquires deterministic multi-lock order for multi-spreadsheet mutations
  - broadens mutation classification with explicit + heuristic coverage
- Added concurrency test suite: `tests/middleware/write-lock-middleware.test.ts`.

---

### ISSUE-244: [FIXED] MEDIUM — Documentation drift: stale production-audit claims vs current code

**Severity**: MEDIUM | **Source**: Pre-phase verification run (2026-03-03)  
**Files**:

- `ServalSheets_Production_Audit.md`
- `MCP_AUDIT_REPORT.md`
- `UNIFIED_IMPROVEMENT_PLAN.md`

Examples of stale claims:

- Per-user rate limiting reported missing (implemented in HTTP path)
- Transaction WAL reported missing (implemented and env-gated)
- Webhook description quality reported minimal (now expanded)
- Several context-size metrics outdated

**Impact**: Planning and prioritization drift; incorrect risk ranking.

**Fix**: Reconcile all active plan/audit docs to validated branch reality and lock a single active source-of-truth set.

**Verification (2026-03-03)**:

- Reconciled and synchronized: `TASKS.md`, `UNIFIED_IMPROVEMENT_PLAN.md`, `MCP_AUDIT_REPORT.md`, `ServalSheets_Production_Audit.md`.
- Added explicit status snapshots and issue transitions in `ISSUES.md`.

---

### ISSUE-245: [FIXED] MEDIUM — Alignment contract test file scope mismatch (stale tracker entry)

**Severity**: MEDIUM | **Source**: Pre-phase verification run (2026-03-03)  
**File**: `tests/contracts/schema-handler-alignment.test.ts`

Issue entry was stale: contract test file already includes `agent`, `compute`, and `connectors` in `TOOLS`.

**Impact**: Tracking drift (false-positive blocker), not an active contract-coverage gap.

**Fix**: Re-verify contract suite and close stale issue entry.

**Verification (2026-03-03)**:

- `npx vitest run tests/contracts/schema-handler-alignment.test.ts` passes (`30/30`).
- Statistics output confirms `Total tools: 25`, `Perfect alignment: 24`, `With documented deviations: 1` (`sheets_core` aliases only).

---

### ISSUE-246: [FIXED] MEDIUM — Webhook durability architecture decision resolved (Redis hard requirement)
...
Decision finalized: `sheets_webhook` remains Redis-backed with Redis as a hard runtime requirement (`redis_required` durability mode). In-memory degraded fallback is intentionally not implemented for webhook lifecycle state.

**Impact**: Repeated churn between "bug" vs "intentional requirement" interpretations.

**Fix**: Record and implement one decision path: keep Redis hard requirement (with explicit operator guidance) or add in-memory degraded fallback.

**Verification (2026-03-03)**:

- Added explicit decision constant: `src/services/webhook-manager.ts` (`WEBHOOK_DURABILITY_MODE = 'redis_required'`).
- Surfaced in handler config errors: `src/handlers/webhooks.ts` (`details.durabilityMode`).
- Confirmed description alignment (`src/schemas/descriptions.ts`) and updated planning docs (`TASKS.md`, `MCP_AUDIT_REPORT.md`, `UNIFIED_IMPROVEMENT_PLAN.md`, `ServalSheets_Production_Audit.md`).
