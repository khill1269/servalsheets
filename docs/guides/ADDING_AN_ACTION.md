...
📊 Analyzing 22 schema files...
  ...
  ...
✅ Total: 25 tools, <current action count> actions
...
Modified (GENERATED - do not edit manually):
  package.json - Description now says "<current action count> actions"
...
Modified:
  ...
  package.json (description: <previous> → <current> actions)
  src/schemas/index.ts (ACTION_COUNT updated)
...
❌ Metadata drift detected in 2 files:
  - src/schemas/index.ts (expected ACTION_COUNT = <current>, found <stale>)
  - package.json (expected "<current action count> actions", found "<stale action count> actions")
```
