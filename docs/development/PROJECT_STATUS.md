...
All verification checks are currently passing. 2026-04-24 session fixed two flat-tool-surface bugs that caused LLM clients to see only 100 of 411 tools:
...
| Actions  | 411            | `src/generated/action-counts.ts` |
...
```bash
npm run audit:coverage      # 411-action coverage test
