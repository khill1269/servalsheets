...
### sheets_appsscript (18) — BaseHandler
...
| ----------------------------- | ------- | ------------------------- | ------------------------------------ |
| ENABLE_BACKGROUND_ANALYSIS    | ON      | Async analysis engine     | Active                               |
| ENABLE_DATAFILTER_BATCH       | ON      | Dynamic range filters     | Active                               |
| ENABLE_TABLE_APPENDS          | ON      | Table append optimization | Active                               |
| ENABLE_PAYLOAD_VALIDATION     | ON      | Pre-Zod validation        | Active                               |
| ENABLE_LEGACY_SSE             | ON      | Legacy SSE transport      | Active                               |
| ENABLE_AGGRESSIVE_FIELD_MASKS | ON      | API response reduction    | Active                               |
| ENABLE_CONDITIONAL_REQUESTS   | ON      | ETag caching              | Active                               |
| ENABLE_AUTO_CONNECTION_RESET  | ON      | HTTP/2 GOAWAY fix         | Active                               |
| ENABLE_REQUEST_MERGING        | ON      | Overlapping range merge   | Active                               |
| ENABLE_PARALLEL_EXECUTOR      | ON      | Parallel batch reads      | Active                               |
| ENABLE_GRANULAR_PROGRESS      | ON      | Fine-grained progress     | Active                               |
| ENABLE_AUDIT_LOGGING          | ON      | Compliance audit trail    | Active                               |
| ENABLE_IDEMPOTENCY            | ON      | Retry-safe dedup          | Active                               |
