# ServalSheets Live State
> Auto-generated by `.serval/generate-state.mjs` — DO NOT EDIT MANUALLY
> Generated: 2026-02-28 04:58:59 UTC

## Version
| Metric | Value | Source |
|--------|-------|--------|
| Tools | 25 | src/schemas/action-counts.ts |
| Actions | 377 | src/schemas/action-counts.ts |
| Package Version | 1.7.0 | package.json |
| Protocol | unknown | src/version.ts |
| TypeScript | ✅ compiles | tsc --noEmit |

## Git
| Metric | Value |
|--------|-------|
| Branch | remediation/phase-1 |
| Last Commit | b7ff65b feat: ServalSheets Definitive Improvement Plan (W1-W6) |
| Working Tree | 91 uncommitted files |

## Tool Inventory
| Tool | Actions |
|------|---------|
| sheets_advanced | 31 |
| sheets_agent | 8 |
| sheets_analyze | 19 |
| sheets_appsscript | 18 |
| sheets_auth | 4 |
| sheets_bigquery | 17 |
| sheets_collaborate | 40 |
| sheets_composite | 20 |
| sheets_compute | 10 |
| sheets_confirm | 5 |
| sheets_connectors | 10 |
| sheets_core | 19 |
| sheets_data | 23 |
| sheets_dependencies | 10 |
| sheets_dimensions | 29 |
| sheets_federation | 4 |
| sheets_fix | 6 |
| sheets_format | 24 |
| sheets_history | 10 |
| sheets_quality | 4 |
| sheets_session | 27 |
| sheets_templates | 8 |
| sheets_transaction | 6 |
| sheets_visualize | 18 |
| sheets_webhook | 7 |

## Monorepo
| Component | Status |
|-----------|--------|
| serval-core | v0.1.0 |
| Adapter layer | airtable-backend.ts, excel-online-backend.ts, google-sheets-backend.ts, index.ts, notion-backend.ts |

## Key Files
| File | Size |
|------|------|
| src/server.ts | 1856 lines |
| src/http-server.ts | 3180 lines |
| src/handlers/base.ts | 1567 lines |
| src/schemas/action-counts.ts | 46 lines |

## Test Health
| Metric | Value |
|--------|-------|
| Status | unknown |
| Summary | Could not run tests (timeout or error) |

## Feature Flags (defaults from src/config/env.ts)
| Flag | Default |
|------|---------|
| ENABLE_BACKGROUND_ANALYSIS | ✅ ON |
| ENABLE_DATAFILTER_BATCH | ✅ ON |
| ENABLE_TABLE_APPENDS | ✅ ON |
| ENABLE_PAYLOAD_VALIDATION | ✅ ON |
| ENABLE_LEGACY_SSE | ❌ OFF |
| ENABLE_TOOLS_LIST_CHANGED_NOTIFICATIONS | ❌ OFF |
| ENABLE_AGGRESSIVE_FIELD_MASKS | ✅ ON |
| ENABLE_CONDITIONAL_REQUESTS | ✅ ON |
| ENABLE_AUTO_CONNECTION_RESET | ✅ ON |
| ENABLE_REQUEST_MERGING | ✅ ON |
| ENABLE_PARALLEL_EXECUTOR | ✅ ON |
| ENABLE_GRANULAR_PROGRESS | ✅ ON |
| ENABLE_METRICS_SERVER | ❌ OFF |
| ENABLE_RBAC | ❌ OFF |
| ENABLE_AUDIT_LOGGING | ✅ ON |
| ENABLE_TENANT_ISOLATION | ❌ OFF |
| ENABLE_IDEMPOTENCY | ✅ ON |
| ENABLE_COST_TRACKING | ❌ OFF |
| ENABLE_BILLING_INTEGRATION | ❌ OFF |
| ENABLE_PREFETCH | ✅ ON |

## Test Infrastructure
| File | Exists |
|------|--------|
| tests/audit/action-coverage-fixtures.ts | ✅ |
| tests/audit/action-coverage.test.ts | ✅ |
| tests/audit/performance-profile.bench.ts | ✅ |
| tests/audit/memory-leaks.test.ts | ✅ |
| scripts/generate-health-snapshot.ts | ✅ |
| scripts/audit-gate.sh | ✅ |
| Test files | 328 total |

## Audit Commands
```bash
npm run audit:coverage   # 315-action coverage test
npm run audit:perf       # Performance benchmarks
npm run audit:memory     # Memory leak detection
npm run audit:gate       # CI gate (7 checks)
npm run audit:snapshot   # Full health report
npm run audit:full       # All of the above
```

## Known Issues
- docs/development/PROJECT_STATUS.md may have stale counts (expected 25/377/1.7.0)
- npm run check:drift hangs/times out
- .claude/agent-memory/servalsheets-validation/MEMORY.md has stale count: TOOL_COUNT = 22 (actual: 25 tools, 377 actions)

## Verification Commands
```bash
npm run verify:safe      # Safe verification (skips lint — use when ESLint OOMs)
npm run verify           # Full verification (typecheck + lint + test + drift)
npm run schema:commit    # After schema changes
npm run test:fast        # Quick test suite
npm run gates            # Full gate pipeline (G0-G5)
```
