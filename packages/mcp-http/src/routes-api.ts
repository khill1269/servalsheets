...
export interface HttpApiRoutesLogger {
...
const defaultLogger: HttpApiRoutesLogger = {
...
export function registerApiRoutes(
  ...
  router.post('/api/formula-eval', async (req: Request, res: Response) => {
    try {
      ...
      log.info('Formula generated via /api/formula-eval', {
      ...
      log.error('Formula evaluation failed', { error: err });
      res.status(500).json({
        error: {
