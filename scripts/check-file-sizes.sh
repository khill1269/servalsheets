...
get_budget_override() {
  ...
  case "$file" in
    ...
    # transaction-manager.ts: ~2473 lines — transaction state machine (WAL extracted to transaction-wal.ts; coalescing logic added in bec50c7b)
    "src/services/transaction-manager.ts")  echo 2550 ;;
    ...
    # cache-invalidation-graph.ts: ~988 lines — invalidation rules for all 409 actions (171-gap burn-down in ff116cc3)
    "src/services/cache-invalidation-graph.ts") echo 1050 ;;
