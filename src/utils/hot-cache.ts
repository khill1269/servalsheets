...
export class HotCache<T = unknown> {
  ...
  get(key: string): T | undefined {
    ...
    if (hotEntry) {
      if (now - hotEntry.timestamp < hotEntry.ttl) {
        ...
        if (this.config.trackStats) this.stats.hotHits++;
    ...
    if (warmEntry) {
      if (now - warmEntry.timestamp < warmEntry.ttl) {
        ...
        if (this.config.trackStats) this.stats.warmHits++;
   ...
   */
  private promoteToHot(key: string, entry: CacheEntry<T>): void {
    ...
    if (this.config.trackStats) this.stats.promotions++;
  }
  ...
  private demoteFromHot(): void {
    ...
    if (lowestKey) {
      ...
      if (this.config.trackStats) this.stats.demotions++;
    }
  }
 ...
 */
export function hotCacheGet<T>(namespace: string, key: string): T | undefined {
  return getHotCache().get(createHotCacheKey(namespace, key)) as T | undefined;
}
 ...
 */
export function hotCacheDelete(namespace: string, key: string): boolean {
 ...
 */
export function hotCacheInvalidateNamespace(namespace: string): number {
