...
const ValidationActionSchema = z.discriminatedUnion('action', [
  z.object({
    action: z.literal('validate'),
    value: z.unknown().describe('Value to validate'),
    rules: z.array(z.string()).optional().describe('Validation rule IDs to apply (all rules if omitted)'),
    context: z.record(z.unknown()).optional().describe('Validation context (spreadsheetId, sheetName, range, etc.)'),
    stopOnFirstError: z.boolean().optional().describe('Stop validation on first error (default: false)'),
  }),
]);

export const SheetsValidationInputSchema = z.object({
  request: ValidationActionSchema,
});
...
export type SheetsValidationOutput = z.infer<typeof SheetsValidationOutputSchema>;
