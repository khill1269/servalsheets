...
export class FastValidationError extends Error {
  ...
  toErrorDetail(): { code: "INVALID_PARAMS"; message: string; details: Record<string, unknown>; retryable: boolean } {
    return {
...
const SPREADSHEET_ACTIONS = new Set([
  "get", "create", "copy", "update_properties", "get_url", "batch_get", "get_comprehensive", "list"
]);
...
// 3. sheets_sheet
const SHEET_ACTIONS = new Set(["add", "delete", "duplicate", "update", "copy_to", "list", "get"]);

export function fastValidateSheet(input: Record<string, unknown>): void {
  assertAction(input["action"], SHEET_ACTIONS);
  assertSpreadsheetId(input["spreadsheetId"]);
  
  const needsSheetId = input["action"] === "delete" || 
...
const VALUES_ACTIONS = new Set([
  "read", "write", "append", "clear", "batch_read", "batch_write", "batch_clear", "find", "replace"
]);
...
export function fastValidateValues(input: Record<string, unknown>): void {
  ...
  switch (input["action"]) {
    case "read":
    case "clear":
      assertString(input["range"], "range");
      break;
    case "write":
    case "append":
      assertString(input["range"], "range");
...
const CELLS_ACTIONS = new Set([
  "add_note", "get_note", "clear_note", "set_validation", "clear_validation",
  "set_hyperlink", "clear_hyperlink", "merge", "unmerge", "get_merges", "cut", "copy"
]);
...
const DIMENSIONS_ACTIONS = new Set([
  ...
  "freeze_rows", "freeze_columns", "group_rows", "group_columns",
  "ungroup_rows", "ungroup_columns", "append_rows", "append_columns"
]);
...
const RULES_ACTIONS = new Set([
  ...
  "list_conditional_formats", "add_data_validation", "clear_data_validation",
  "list_data_validations", "add_preset_rule"
]);
...
const CHARTS_ACTIONS = new Set([
  "create", "update", "delete", "list", "get", "move", "resize", "update_data_range", "export"
]);
...
// 10. sheets_pivot
const PIVOT_ACTIONS = new Set(["create", "update", "delete", "list", "get", "refresh"]);
...
const FILTER_SORT_ACTIONS = new Set([
  ...
  "sort_range", "create_filter_view", "update_filter_view", "delete_filter_view",
  "list_filter_views", "get_filter_view", "create_slicer", "update_slicer",
  "delete_slicer", "list_slicers"
]);
...
const SHARING_ACTIONS = new Set([
  ...
  "get_permission", "transfer_ownership", "set_link_sharing", "get_sharing_link"
]);
...
const COMMENTS_ACTIONS = new Set([
  "add", "update", "delete", "list", "get", "resolve", "reopen", "add_reply", "update_reply", "delete_reply"
]);
...
const VERSIONS_ACTIONS = new Set([
  "list_revisions", "get_revision", "restore_revision", "keep_revision",
  "create_snapshot", "list_snapshots", "restore_snapshot", "delete_snapshot",
  "compare", "export_version"
]);
...
const ANALYSIS_ACTIONS = new Set([
  "data_quality", "formula_audit", "structure_analysis", "statistics",
  "correlations", "summary", "dependencies", "compare_ranges",
  "detect_patterns", "column_analysis", "suggest_templates", "generate_formula", "suggest_chart"
]);
