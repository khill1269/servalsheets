...
export const TOOL_DESCRIPTIONS_MINIMAL: Record<string, string> = {
  ...
  sheets_data: `DATA - Read/write cell values (${ACTION_COUNTS['sheets_data']} actions). read, write, append, batch_read, batch_write, find_replace, cross_read, cross_query. NOT for formatting (use sheets_format). NOT for sheet structure (use sheets_core). MISTAKES: find_replace ≠ targeted update (use write); append is NOT idempotent. SAFETY: batch_read/batch_write cost the same as a single call — always prefer batch for 3+ ranges. cross_write requires confirmation before overwrite.`,
  ...
  sheets_composite: `COMPOSITE - High-level multi-step workflows (${ACTION_COUNTS['sheets_composite']} actions). import_csv, import_xlsx, smart_append, deduplicate, setup_sheet, generate_sheet, audit_sheet, publish_report, data_pipeline, migrate_spreadsheet. WHEN: import CSV/xlsx, setup/generate sheet, deduplicate. 60-80% API savings. NOT for single cell reads/writes (use sheets_data). NOT for formatting only (use sheets_format). SAFETY: deduplicate permanently deletes matching rows. data_pipeline can include mutations — use dryRun:true first.`,
