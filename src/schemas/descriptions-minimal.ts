...
export const TOOL_DESCRIPTIONS_MINIMAL: Record<string, string> = {
  ...
  sheets_data: `DATA - Read/write cell values (${ACTION_COUNTS['sheets_data']} actions). read, write, append, batch_read, batch_write, find_replace, cross_read, cross_query. Range: "Sheet1!A1:D10". NOT for formatting (use sheets_format). NOT for sheet structure (use sheets_core). MISTAKES: find_replace is not targeted update (use write); append is not idempotent. SAFETY: batch_read/batch_write cost the same as a single call — prefer batch for 3+ ranges. cross_write requires confirmation before overwrite.`,
  ...
  sheets_composite: `COMPOSITE - High-level multi-step workflows (${ACTION_COUNTS['sheets_composite']} actions). import_csv, import_xlsx, smart_append, deduplicate, setup_sheet, generate_sheet, audit_sheet, publish_report, data_pipeline, migrate_spreadsheet. WHEN: import CSV/XLSX, set up/generate sheet, deduplicate, publish report. 60-80% API savings. NOT for single cell reads/writes (use sheets_data). NOT for formatting only (use sheets_format). SAFETY: deduplicate deletes rows; data_pipeline can mutate — use dryRun:true first.`,
  ...
  sheets_appsscript: `APPSSCRIPT - Google Apps Script automation (${ACTION_COUNTS['sheets_appsscript']} actions). create, update_content, create_version, deploy, run, list_processes, get_metrics. Use for custom menus, mail merge, print setup, and ScriptApp trigger helpers shipped via update_content + deploy. USER OAuth only. NOT for simple data reads/writes (use sheets_data). SAFETY: deploy can expose script; review update_content before deploy. run executes arbitrary Apps Script code.`,
