...
export const TOOL_DESCRIPTIONS_MINIMAL: Record<string, string> = {
  ...
  sheets_appsscript: `APPSSCRIPT - Google Apps Script automation (${ACTION_COUNTS['sheets_appsscript']} actions). create, update_content, create_version, deploy, run, list_processes, get_metrics. Use for: print setup, triggers, custom menus, mail merge. USER OAuth only. NOT for simple data reads/writes (use sheets_data). NOT for formulas (use sheets_compute or sheets_data.write).`,

  sheets_webhook: `WEBHOOK - Change notifications (${ACTION_COUNTS['sheets_webhook']} actions). register, unregister, list, test, watch_changes. Requires Redis backend + HTTPS endpoint. NOT for reading data (use sheets_data). NOT for polling changes (use sheets_history.timeline).`,
