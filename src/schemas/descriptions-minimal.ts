 ...
 * Target: ~100-150 tokens per tool (vs ~350-500 in full mode)
 *
...
export const TOOL_DESCRIPTIONS_MINIMAL: Record<string, string> = {
  ...
  sheets_core: `CORE - Spreadsheet/sheet management (${ACTION_COUNTS['sheets_core']} actions). create, get, list_sheets, add_sheet, update_sheet, delete_sheet, batch_get, copy. NOT for cell values (use sheets_data). NOT for formatting (use sheets_format). SAFETY: delete_sheet and clear_sheet are permanent — call sheets_history.create_snapshot first. Use sheets_composite.setup_sheet (not create+add_sheet) when you need headers+formatting in one step.`,

  sheets_data: `DATA - Read/write cell values (${ACTION_COUNTS['sheets_data']} actions). read, write, append, batch_read, batch_write, find_replace, cross_read, cross_query. Range: "Sheet1!A1:D10". NOT for formatting (use sheets_format). NOT for sheet structure (use sheets_core). MISTAKES: find_replace != targeted update (use write); append is NOT idempotent. SAFETY: batch_read/batch_write cost the same as a single call — always prefer batch for 3+ ranges. cross_write requires confirmation before overwrite.`,
  ...
  sheets_composite: `COMPOSITE - High-level multi-step workflows (${ACTION_COUNTS['sheets_composite']} actions). import_csv, import_xlsx, smart_append, deduplicate, setup_sheet, generate_sheet, audit_sheet, publish_report, data_pipeline, migrate_spreadsheet. WHEN: "import a CSV", "set up sheet", "deduplicate". 60-80% API savings. NOT for single cell reads/writes (use sheets_data). NOT for formatting only (use sheets_format). SAFETY: deduplicate permanently deletes matching rows. data_pipeline can include mutations — use dryRun:true first.`,
  ...
  sheets_bigquery: `BIGQUERY - Connected Sheets and SQL (${ACTION_COUNTS['sheets_bigquery']} actions). query, connect_table, import_from_bigquery, export_to_bigquery, connect_looker. Requires BigQuery API enabled + service account. NOT for in-sheet calculations (use sheets_compute). NOT for CSV import (use sheets_composite.import_csv). SAFETY: export_to_bigquery overwrites target table by default. import_from_bigquery overwrites the destination range.`,

  sheets_appsscript: `APPSSCRIPT - Google Apps Script automation (${ACTION_COUNTS['sheets_appsscript']} actions). create, update_content, create_version, deploy, run, list_processes, get_metrics. Use for custom menus, mail merge, print setup, and ScriptApp-based trigger code shipped via update_content + deploy. Trigger CRUD hidden (no trigger-management REST API). USER OAuth only. NOT for simple data reads/writes (use sheets_data). SAFETY: deploy makes script public — review update_content before deploy. run executes arbitrary Apps Script code.`,
  ...
  sheets_agent: `AGENT - Autonomous multi-step execution (${ACTION_COUNTS['sheets_agent']} actions). plan, execute, execute_step, observe, rollback, get_status, list_plans, resume. Use for complex tasks requiring 3+ coordinated tool calls. Checkpoint-based rollback. NOT for single operations (call the tool directly). NOT for analysis (use sheets_analyze). SAFETY: execute runs all plan steps autonomously — call observe first to checkpoint. rollback requires a prior observe checkpoint.`,
