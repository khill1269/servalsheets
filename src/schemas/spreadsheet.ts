...
export const SheetSpreadsheetInputSchema = z.object({
  ...
  spreadsheetId: SpreadsheetIdSchema.optional().describe("Spreadsheet ID from URL (required for: get, copy, update_properties, get_url, get_comprehensive)"),
  includeGridData: z.boolean().optional().default(false).describe("Include cell data in response (get, get_comprehensive)"),
  ranges: z.array(z.string()).optional().describe("Specific ranges to fetch if includeGridData=true (get only)"),

  // Fields for CREATE action
  title: z.string().min(1).max(255).optional().describe("Spreadsheet title (required for: create; optional for: update_properties)"),
  locale: z.string().optional().default("en_US").describe("Locale for formatting (create, update_properties)"),
  timeZone: z.string().optional().describe("Time zone like America/New_York (create, update_properties)"),
  ...
  })).optional().describe("Initial sheets/tabs to create (create only)"),

  // Fields for COPY action
  destinationFolderId: z.string().optional().describe("Google Drive folder ID to copy into (copy only)"),
  newTitle: z.string().optional().describe("Title for the copied spreadsheet (copy only)"),

  // Fields for UPDATE_PROPERTIES action
  autoRecalc: z.enum(["ON_CHANGE", "MINUTE", "HOUR"]).optional().describe("Automatic recalculation frequency (update_properties only)"),

  // Fields for BATCH_GET action
  spreadsheetIds: z.array(SpreadsheetIdSchema).min(1).max(100).optional().describe("Array of spreadsheet IDs 1-100 (required for: batch_get)"),

  // Fields for GET_COMPREHENSIVE action
  maxRowsPerSheet: z.number().int().positive().optional().default(100).describe("Max rows per sheet if includeGridData=true (get_comprehensive only)"),

  // Fields for LIST action
  maxResults: z.number().int().positive().optional().default(100).describe("Maximum number of spreadsheets to return (list only)"),
  query: z.string().optional().describe("Search query to filter spreadsheets (list only)"),
  orderBy: z.enum(["createdTime", "modifiedTime", "name", "viewedByMeTime"]).optional().default("modifiedTime").describe("How to order results (list only)"),
}).refine((data) => {
...
export type SpreadsheetGetInput = SheetsSpreadsheetInput & { action: "get"; spreadsheetId: string };
export type SpreadsheetCreateInput = SheetsSpreadsheetInput & { action: "create"; title: string };
export type SpreadsheetCopyInput = SheetsSpreadsheetInput & { action: "copy"; spreadsheetId: string };
export type SpreadsheetUpdatePropertiesInput = SheetsSpreadsheetInput & { action: "update_properties"; spreadsheetId: string };
export type SpreadsheetGetUrlInput = SheetsSpreadsheetInput & { action: "get_url"; spreadsheetId: string };
export type SpreadsheetBatchGetInput = SheetsSpreadsheetInput & { action: "batch_get"; spreadsheetIds: string[] };
export type SpreadsheetGetComprehensiveInput = SheetsSpreadsheetInput & { action: "get_comprehensive"; spreadsheetId: string };
