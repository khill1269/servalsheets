...
export const SheetsCellsInputSchema = z
  .object({
...
export type CellsAddNoteInput = SheetsCellsInput & {
  action: "add_note";
  spreadsheetId: string;
  cell: string;
  note: string;
};
export type CellsGetNoteInput = SheetsCellsInput & {
  action: "get_note";
  spreadsheetId: string;
  cell: string;
};
export type CellsClearNoteInput = SheetsCellsInput & {
  action: "clear_note";
  spreadsheetId: string;
  cell: string;
};
export type CellsSetValidationInput = SheetsCellsInput & {
  action: "set_validation";
  spreadsheetId: string;
  range: string | { semantic: string };
  validation: z.infer<typeof DataValidationSchema>;
};
export type CellsClearValidationInput = SheetsCellsInput & {
  action: "clear_validation";
  spreadsheetId: string;
  range: string | { semantic: string };
};
export type CellsSetHyperlinkInput = SheetsCellsInput & {
  action: "set_hyperlink";
  spreadsheetId: string;
  cell: string;
  url: string;
};
export type CellsClearHyperlinkInput = SheetsCellsInput & {
  action: "clear_hyperlink";
  spreadsheetId: string;
  cell: string;
};
export type CellsMergeInput = SheetsCellsInput & {
  action: "merge";
  spreadsheetId: string;
  range: string | { semantic: string };
};
export type CellsUnmergeInput = SheetsCellsInput & {
  action: "unmerge";
  spreadsheetId: string;
  range: string | { semantic: string };
};
export type CellsGetMergesInput = SheetsCellsInput & {
  action: "get_merges";
  spreadsheetId: string;
  sheetId: number;
};
export type CellsCutInput = SheetsCellsInput & {
  action: "cut";
  spreadsheetId: string;
  source: string | { semantic: string };
  destination: string;
};
export type CellsCopyInput = SheetsCellsInput & {
  action: "copy";
  spreadsheetId: string;
  source: string | { semantic: string };
  destination: string;
