...
export class RangeResolver {
  ...
  private async resolveSemantic(
    ...
    const matches: Array<{ header: string; index: number; confidence: number }> = [];
    ...
    return {
      ...
      resolution: {
        ...
        alternatives: matches.length > 1 
          ? matches.filter(m => m !== bestMatch).map(m => ({
  ...
  private a1ToGridRange(sheetId: number, a1: string): GridRange {
    ...
    return {
      ...
      endColumnIndex: endCol ? this.letterToColumnIndex(endCol) + 1 : 
                      startCol ? this.letterToColumnIndex(startCol) + 1 : undefined,
      startRowIndex: startRow ? parseInt(startRow) - 1 : undefined,
      endRowIndex: endRow ? parseInt(endRow) : 
                   startRow ? parseInt(startRow) : undefined,
    };
  }
