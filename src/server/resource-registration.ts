...
export interface RegisterServerResourcesOptions {
  readonly deferKnowledgeResources?: boolean;
  readonly includeTimeTravelResources?: boolean;
  readonly toolsListSyncReason?: string;
}
...
export async function registerServerResources(params: {
  ...
  options?: RegisterServerResourcesOptions;
}): Promise<void> {
  const { server, googleClient, context, options } = params;
  registerServalSheetsResources(server, googleClient);

  const useDeferred =
    options?.deferKnowledgeResources ??
    (process.env['DISABLE_KNOWLEDGE_RESOURCES'] === 'true' || DEFER_SCHEMAS);
  if (useDeferred) {
  ...
  if (options?.includeTimeTravelResources ?? true) {
  ...
  if (getEnv().ENABLE_TOOLS_LIST_CHANGED_NOTIFICATIONS) {
    resourceNotifications.syncToolList(
      TOOL_DEFINITIONS.map((tool) => tool.name),
      {
        ...
        reason: options?.toolsListSyncReason ?? 'resource initialization completed',
      }
    );
  }
}
