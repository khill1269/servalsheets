...
export function createOptimizedHandlerMap(
  ...
  // Core tools
  map.set("sheets_spreadsheet", {
    fastValidator: fastValidateSpreadsheet,
    zodParser: (input) => SheetSpreadsheetInputSchema.parse(input),
    handler: (v) => handlers.spreadsheet.handle(v as Parameters<typeof handlers.spreadsheet.handle>[0]),
  });

  map.set("sheets_sheet", {
    fastValidator: fastValidateSheet,
    zodParser: (input) => SheetsSheetInputSchema.parse(input),
    handler: (v) => handlers.sheet.handle(v as Parameters<typeof handlers.sheet.handle>[0]),
  });

  map.set("sheets_values", {
    fastValidator: fastValidateValues,
    zodParser: (input) => SheetsValuesInputSchema.parse(input),
    handler: (v) => handlers.values.handle(v as Parameters<typeof handlers.values.handle>[0]),
  });

  map.set("sheets_cells", {
    fastValidator: fastValidateCells,
    zodParser: (input) => SheetsCellsInputSchema.parse(input),
    handler: (v) => handlers.cells.handle(v as Parameters<typeof handlers.cells.handle>[0]),
  });
  ...
  map.set("sheets_dimensions", {
    ...
    handler: (v) => handlers.dimensions.handle(v as Parameters<typeof handlers.dimensions.handle>[0]),
  });

  map.set("sheets_rules", {
    fastValidator: fastValidateRules,
    zodParser: (input) => SheetsRulesInputSchema.parse(input),
    handler: (v) => handlers.rules.handle(v as Parameters<typeof handlers.rules.handle>[0]),
  });

  map.set("sheets_charts", {
    fastValidator: fastValidateCharts,
    zodParser: (input) => SheetsChartsInputSchema.parse(input),
    handler: (v) => handlers.charts.handle(v as Parameters<typeof handlers.charts.handle>[0]),
  });

  map.set("sheets_pivot", {
    fastValidator: fastValidatePivot,
    zodParser: (input) => SheetsPivotInputSchema.parse(input),
    handler: (v) => handlers.pivot.handle(v as Parameters<typeof handlers.pivot.handle>[0]),
  });

  map.set("sheets_filter_sort", {
    fastValidator: fastValidateFilterSort,
    zodParser: (input) => SheetsFilterSortInputSchema.parse(input),
    handler: (v) => handlers.filterSort.handle(v as Parameters<typeof handlers.filterSort.handle>[0]),
  });

  map.set("sheets_sharing", {
    fastValidator: fastValidateSharing,
    zodParser: (input) => SheetsSharingInputSchema.parse(input),
    handler: (v) => handlers.sharing.handle(v as Parameters<typeof handlers.sharing.handle>[0]),
  });

  map.set("sheets_comments", {
    fastValidator: fastValidateComments,
    zodParser: (input) => SheetsCommentsInputSchema.parse(input),
    handler: (v) => handlers.comments.handle(v as Parameters<typeof handlers.comments.handle>[0]),
  });

  map.set("sheets_versions", {
    fastValidator: fastValidateVersions,
    zodParser: (input) => SheetsVersionsInputSchema.parse(input),
    handler: (v) => handlers.versions.handle(v as Parameters<typeof handlers.versions.handle>[0]),
  });
  ...
  map.set("sheets_transaction", {
    ...
    handler: (v) => handlers.transaction.handle(v as Parameters<typeof handlers.transaction.handle>[0]),
  });

  map.set("sheets_validation", {
    fastValidator: fastValidateValidation,
    zodParser: (input) => SheetsValidationInputSchema.parse(input),
    handler: (v) => handlers.validation.handle(v as Parameters<typeof handlers.validation.handle>[0]),
  });

  map.set("sheets_conflict", {
    fastValidator: fastValidateConflict,
    zodParser: (input) => SheetsConflictInputSchema.parse(input),
    handler: (v) => handlers.conflict.handle(v as Parameters<typeof handlers.conflict.handle>[0]),
  });

  map.set("sheets_impact", {
    fastValidator: fastValidateImpact,
    zodParser: (input) => SheetsImpactInputSchema.parse(input),
    handler: (v) => handlers.impact.handle(v as Parameters<typeof handlers.impact.handle>[0]),
  });
