...
/**
 * Get only the always-loaded (non-deferred) flat tools.
 * These are the tools that appear in the LLM's initial context.
 */
export function getAlwaysLoadedFlatTools(): FlatToolDefinition[] {
  return getFlatToolRegistry().filter((t) => !t.deferLoading);
}

/**
 * Get all deferred flat tools.
 * These are discovered via tool_search / sheets_discover.
 */
export function getDeferredFlatTools(): FlatToolDefinition[] {
  return getFlatToolRegistry().filter((t) => t.deferLoading);
}
...
export function searchFlatToolsByPattern(
  ...
  try {
    ...
    return getFlatToolRegistry()
      .filter((t) => regex.test(t.name) || regex.test(t.description))
      .slice(0, maxResults);
  } catch {
    // Invalid regex — fall back to substring match
    const lower = pattern.toLowerCase();
    return getFlatToolRegistry()
      .filter((t) => t.name.includes(lower) || t.description.toLowerCase().includes(lower))
      .slice(0, maxResults);
  }
}

/**
 * Get flat tool registry statistics.
 */
export function getFlatRegistryStats(): {
  total: number;
  alwaysLoaded: number;
  deferred: number;
  ...
  for (const tool of registry) {
    const domain = getDomainPrefix(tool.parentTool);
 ...
 */
export function clearFlatRegistryCache(): void {
  flatRegistryCache = null;
}
