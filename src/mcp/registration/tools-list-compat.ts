...
const ALWAYS_LOADED_SCHEMAS: Record<string, Record<string, unknown>> = {
  ...
  'sheets_session.update_preferences': {
    type: 'object',
    properties: {
      confirmationLevel: { type: 'string', enum: ['always', 'destructive', 'never'], description: 'When to ask for confirmation' },
  ...
  'sheets_data.write': {
    ...
    properties: {
      spreadsheetId: { type: 'string', description: 'The Google Sheets spreadsheet ID' },
      range: { type: 'string', description: 'A1 notation range (e.g., Sheet1!A1:D10)' },
      values: { type: 'array', items: { type: 'array' }, description: 'Array of row arrays to write' },
      valueInputOption: { type: 'string', enum: ['RAW', 'USER_ENTERED'], description: 'How input values should be interpreted' },
      includeValuesInResponse: { type: 'boolean', description: 'Include written values in response' },
    },
  ...
  'sheets_data.append': {
    ...
    properties: {
      ...
      values: { type: 'array', items: { type: 'array' }, description: 'Array of row arrays to append' },
      valueInputOption: { type: 'string', enum: ['RAW', 'USER_ENTERED'], description: 'How input values should be interpreted' },
      insertDataOption: { type: 'string', enum: ['OVERWRITE', 'INSERT_ROWS'], description: 'How to insert data' },
    },
    required: ['spreadsheetId', 'values'],
  },
  'sheets_core.list': {
  ...
  'sheets_core.get': {
    ...
    properties: {
      ...
      ranges: { type: 'array', items: { type: 'string' }, description: 'Ranges to include grid data for' },
      response_format: { type: 'string', enum: ['full', 'compact', 'preview'] },
    },
    required: ['spreadsheetId'],
  },
...
function buildFlatToolListEntries(): Record<string, unknown>[] {
  ...
  return flatTools.map((flat: FlatToolDefinition) => {
    ...
    if (detailedSchema) {
      ...
      if (flat.parentTool === 'sheets_data' || flat.parentTool === 'sheets_format' ||
