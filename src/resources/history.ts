...
export function registerHistoryResources(server: McpServer): number {
  ...
  server.registerResource(
    ...
    {
      description:
        "Last 100 operations with full details for debugging, undo, and audit",
  ...
  server.registerResource(
    ...
    {
      description:
        "Operation statistics: success rate, average duration, most common tools",
      mimeType: "application/json",
    },
    async (uri) => {
      try {
              ...
              text: JSON.stringify(
                {
                  ...
                  failures,
                  note: count
                    ? `Showing last ${count} failures`
                    : "Showing all failures",
                },
                null,
                2,
              ),
