...
/**
 * Annotates a plan generated by AI to mark it as AI-generated.
 * This adds a metadata flag used for auditing and confidence scoring.
 */
export function annotateAIGeneratedDraftPlan(plan: { metadata?: Record<string, unknown> }): void {
  if (!plan.metadata) {
    (plan as Record<string, unknown>)['metadata'] = {};
  }
  (plan.metadata as Record<string, unknown>)['aiGenerated'] = true;
  (plan.metadata as Record<string, unknown>)['generatedAt'] = new Date().toISOString();
}
