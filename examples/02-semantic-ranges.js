#!/usr/bin/env node
/**
 * ServalSheets Example 2: Semantic Range Queries
 *
 ...
 */
async function findColumnByHeader(sheets, spreadsheetId, sheetName, headerName) {
  ...
  try {
    ...
    console.error(`✗ Failed to find column: ${error.message}`);
    throw error;
  }
}
 ...
 */
async function readByHeader(sheets, spreadsheetId, sheetName, headerName, includeHeader = false) {
  ...
  try {
    ...
    console.error(`✗ Failed to read by header: ${error.message}`);
    throw error;
  }
}
 ...
 */
async function readMultipleByHeaders(sheets, spreadsheetId, sheetName, headerNames, includeHeaders = false) {
  ...
  try {
    ...
    console.error(`✗ Failed to read multiple headers: ${error.message}`);
    throw error;
  }
}
 ...
 */
async function readNamedRange(sheets, spreadsheetId, rangeName) {
  ...
  try {
    ...
    console.log(`  Actual range: ${response.data.range}`);
    ...
    if (error.message.includes('Unable to parse range')) {
      ...
      console.error(`✗ Failed to read named range: ${error.message}`);
    }
    throw error;
  }
}
...
async function main() {
  console.log('=== ServalSheets Example: Semantic Range Queries ===\n');
