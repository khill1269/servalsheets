#!/usr/bin/env node
/**
 * ServalSheets Example 1: Basic Read/Write Operations
 *
 ...
 */
async function readData(sheets, spreadsheetId, range) {
 ...
 */
async function writeData(sheets, spreadsheetId, range, values) {
  ...
  try {
    ...
    console.log(`  Rows affected: ${response.data.updatedRows}`);
    console.log(`  Columns affected: ${response.data.updatedColumns}`);
...
async function clearData(sheets, spreadsheetId, range) {
  ...
  try {
    ...
    console.log(`✓ Successfully cleared ${response.data.clearedRange}`);
    return response.data;
  } catch (error) {
...
async function main() {
  console.log('=== ServalSheets Example: Basic Read/Write ===\n');
