            ...
            switch (req.action) {
                case 'begin': {
                    ...
                    if (!req.spreadsheetId) {
                        throw new ValidationError('spreadsheetId is required for begin action', 'spreadsheetId');
                    }
                ...
                case 'queue': {
                    ...
                    if (!req.transactionId || !req.operation) {
                        throw new ValidationError('transactionId and operation are required for queue action', 'transactionId');
                    }
                    ...
                    if (tx.operations.length > 50) {
                        warnings.push(`Large transaction (${tx.operations.length} operations). Consider splitting into multiple smaller transactions for better reliability and easier debugging.`);
                    }
                    else if (tx.operations.length > 20) {
                        warnings.push(`Transaction size is growing (${tx.operations.length} operations). Maximum recommended size is 50 operations.`);
                    }
                    response = {
                        success: true,
                        action: 'queue',
                ...
                case 'commit': {
                    ...
                    if (!req.transactionId) {
                        throw new ValidationError('transactionId is required for commit action', 'transactionId');
                    }
                ...
                case 'rollback': {
                    // Type assertion after validation
                    if (!req.transactionId) {
                        throw new ValidationError('transactionId is required for rollback action', 'transactionId');
                    }
                ...
                case 'status': {
                    // Type assertion after validation
                    if (!req.transactionId) {
                        throw new ValidationError('transactionId is required for status action', 'transactionId');
                    }
                    const tx = transactionManager.getTransaction(req.transactionId);
                    response = {
                        success: true,
                ...
                case 'list': {
                    ...
                    response = {
                        ...
                        error: {
                            code: 'UNIMPLEMENTED',
                            message: 'Transaction listing is not yet implemented. TransactionManager does not expose listTransactions() method.',
                            retryable: false,
                            suggestedFix: 'Use status action with a specific transactionId to check individual transaction status.',
                            details: {
                ...
                default: {
                    ...
                    response = {
                        ...
                        error: {
                            code: 'INVALID_PARAMS',
                            message: `Unsupported action: ${req.action}`,
                            retryable: false,
                        },
                    };
                }
