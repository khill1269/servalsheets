...
export class AnalysisHandler extends BaseHandler {
    ...
    async handle(input) {
        ...
        const inferredRequest = this.inferRequestParameters(input);
        try {
    ...
    async handleDataQuality(input) {
        ...
        if (input.useAI && this.context.samplingServer) {
            const samplingSupport = checkSamplingSupport(this.context.samplingServer.getClientCapabilities());
            if (samplingSupport.supported) {
                try {
                    ...
                    const issues = aiIssues.map((issue) => ({
                        type: issue.type.toUpperCase().replace(/_/g, '_'),
        ...
        for (let col = 0; col < colCount; col++) {
            ...
            if (ws !== undefined) {
                issues.push({
                    type: /^\s+/.test(ws) ? 'LEADING_WHITESPACE' : 'TRAILING_WHITESPACE',
                    severity: 'low',
                    location: `col:${col + 1}`,
    ...
    async handleFormulaAudit(input) {
        if (!input.spreadsheetId) {
            return this.error({
                code: 'INVALID_PARAMS',
                message: 'spreadsheetId is required',
    ...
    async handleStructure(input) {
        ...
        for (const sheet of sheets) {
            ...
            if (!isHidden &&
                (gridProps?.frozenColumnCount ?? 0) === 0 &&
    ...
    async handleCorrelations(input) {
        if (!input.spreadsheetId || !input.range) {
            return this.error({
                code: 'INVALID_PARAMS',
        ...
        const matrix = Array.from({ length: colCount }, () => Array(colCount).fill(1));
        for (let i = 0; i < colCount; i++) {
            const colI = rows.map((r) => r[i]).filter((v) => typeof v === 'number');
            for (let j = i + 1; j < colCount; j++) {
                const colJ = rows.map((r) => r[j]).filter((v) => typeof v === 'number');
    ...
    async handleDependencies(_input) {
        /**
    ...
    async handleCompareRanges(input) {
            ...
            for (let c = 0; c < maxCols; c++) {
                ...
                if (v1 !== v2) {
                    differences.push({
                        ...
                        value1: v1 ?? null,
                        value2: v2 ?? null,
                        type: v1 === undefined || v2 === undefined
                            ? 'missing'
                            : typeof v1 !== typeof v2
                                ? 'type'
                                : 'value',
    ...
    async handleDetectPatterns(input) {
        if (!input.spreadsheetId || !input.range) {
            return this.error({
                code: 'INVALID_PARAMS',
...
Keep response concise and actionable.`,
                    ...
                    const contentArray = Array.isArray(aiResponse.content)
                        ? aiResponse.content
                        : [aiResponse.content];
    ...
    async handleColumnAnalysis(input) {
        if (!input.spreadsheetId || !input.range) {
            return this.error({
                code: 'INVALID_PARAMS',
...
Keep response concise and actionable.`,
    ...
    analyzeCorrelationsData(values) {
        const correlations = [];
        if (values.length === 0 || !values[0])
            return correlations;
        const columnCount = values[0].length;
    ...
    analyzeDistribution(columnData) {
    ...
    checkColumnQuality(columnData, dataType) {
    ...
    // ============================================================
    async handleSuggestTemplates(input) {
        const logger = getRequestLogger();
...
Format as JSON array with this structure:
{
  ...
  "reasoning": "Why these templates were suggested"
    ...
    async handleGenerateFormula(input) {
        const logger = getRequestLogger();
        if (!input.spreadsheetId || !input.description) {
            return this.error({
                code: 'INVALID_PARAMS',
