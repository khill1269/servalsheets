...
import { getSamplingAnalysisService, buildAnalysisSamplingRequest, buildFormulaSamplingRequest, buildChartSamplingRequest, parseAnalysisResponse, } from '../services/sampling-analysis.js';
...
export class AnalyzeHandler {
    context;
    sheetsApi;
    constructor(context, sheetsApi, _options) {
    ...
    convertRangeInput(range) {
        ...
        if ('a1' in range)
            return { a1: range.a1 };
        if ('namedRange' in range)
            return { a1: range.namedRange };
    ...
    async resolveSheetId(spreadsheetId, sheetName) {
        if (!sheetName)
            return 0;
        ...
        try {
            ...
            switch (req.action) {
                case 'analyze_data': {
                    ...
                    const decision = router.route({ request: analyzeInput }, metadata);
                    ...
                    switch (decision.path) {
                        case 'fast': {
                            ...
                            const sampleResult = await tieredRetrieval.getSample(analyzeInput.spreadsheetId, sheetId, 100 // Default sample size
                            );
                            // Extract sample data
                            ...
                            const { analyzeTrends, detectAnomalies, analyzeCorrelationsData } = await import('../analysis/helpers.js');
                        ...
                        case 'ai': {
                            ...
                            const useFullData = 'analysisTypes' in analyzeInput &&
                            ...
                            const dataResult = useFullData
                                ? await tieredRetrieval.getFull(analyzeInput.spreadsheetId, sheetId)
                                : await tieredRetrieval.getSample(analyzeInput.spreadsheetId, sheetId, 200 // Larger sample for AI analysis
                                );
                            ...
                            const targetSheet = sheetId !== undefined
                            ...
                            const samplingRequest = buildAnalysisSamplingRequest(data, {
                                ...
                                analysisTypes: analyzeInput.analysisTypes,
                            ...
                            if (!parsed.success || !parsed.result) {
                                analysisService.recordFailure(analyzeInput.analysisTypes);
                                response = {
                                    success: false,
                                    error: {
                                        code: 'PARSE_ERROR',
                            ...
                            analysisService.recordSuccess(analyzeInput.analysisTypes, duration);
                            response = {
                                success: true,
                                action: 'analyze_data',
                        ...
                        case 'streaming': {
                            ...
                            const streamingAnalyzer = new StreamingAnalyzer(this.sheetsApi, tieredRetrieval, 1000 // 1000 rows per chunk
                            );
                            // Execute streaming analysis with progress tracking
                            const streamingResult = await streamingAnalyzer.execute(analyzeInput.spreadsheetId, sheetId, metadata, async (chunk) => {
                                // Progress callback
                            ...
                            response = {
                                ...
                                overallQualityScore: Math.max(50, 100 -
                                    Math.floor((streamingResult.aggregatedResults.nullCount /
                ...
                case 'generate_formula': {
                    ...
                    // Build sampling request
                    const formulaSheetName = formulaInput.range && 'sheetName' in formulaInput.range
                    ...
                    try {
                        ...
                        if (!jsonMatch)
                            throw new DataError('No JSON in response - model returned invalid format', 'DATA_ERROR', false);
                ...
                case 'suggest_visualization': {
                    ...
                    try {
                        ...
                        const chartRecommendations = parsed.recommendations?.map((r) => {
                ...
                case 'detect_patterns': {
                    ...
                    const { detectAnomalies, analyzeTrends, analyzeCorrelationsData } = await import('../analysis/helpers.js');
                    try {
                ...
                case 'analyze_structure': {
                    ...
                    try {
                        ...
                        // Calculate totals
                        const totalRows = sheets.reduce((sum, sheet) => sum + (sheet.properties?.gridProperties?.rowCount ?? 0), 0);
                        const totalColumns = sheets.reduce((sum, sheet) => sum + (sheet.properties?.gridProperties?.columnCount ?? 0), 0);
                        const structure = {
                            ...
                            namedRanges: namedRanges.map((nr) => ({
                                ...
                                range: nr.range?.startRowIndex !== undefined && nr.range.startRowIndex !== null
                ...
                case 'analyze_quality': {
                    ...
                    try {
                        ...
                        const avgCompleteness = columnResults.reduce((sum, col) => sum + col.completeness, 0) / columnResults.length;
                        const avgConsistency = columnResults.reduce((sum, col) => sum + col.consistency, 0) / columnResults.length;
                        const overallScore = (avgCompleteness + avgConsistency) / 2;
                        // Build issues array
                        const issues = columnResults.flatMap((col) => col.issues.map((issue) => ({
                ...
                case 'analyze_performance': {
                    ...
                    try {
                        ...
                        const performance = {
                            ...
                            complexFormulas: sheets.reduce((sum, sheet) => sum +
                                (sheet.data?.[0]?.rowData?.filter((row) => row.values?.some((cell) => cell.userEnteredValue?.formulaValue)).length ?? 0), 0),
                        ...
                        response = {
                            ...
                            performance: {
                                overallScore,
                                recommendations,
                                estimatedImprovementPotential: recommendations.length > 0
                ...
                case 'analyze_formulas': {
                    ...
                    try {
                        ...
                        const { findVolatileFormulas, analyzeFormulaComplexity, detectCircularRefs, generateOptimizations, } = await import('../analysis/formula-helpers.js');
                        // Analyze formulas
                        ...
                        const optimizationOpportunities = formulaInput.includeOptimizations !== false ? generateOptimizations(formulas) : [];
                        const duration = Date.now() - startTime;
                        response = {
                            success: true,
                            ...
                            formulaAnalysis: {
                                ...
                                circularReferences: circularRefs.length > 0
                                    ? circularRefs.map((c) => ({
                                        cells: c.cells,
                                        chain: c.chain,
                                    }))
                ...
                case 'query_natural_language': {
                    ...
                    try {
                        ...
                        const { detectQueryIntent, buildNLQuerySamplingRequest, validateQuery } = await import('../analysis/conversational-helpers.js');
                ...
                case 'explain_analysis': {
                    ...
                    if (!this.context.server) {
                        response = {
                            ...
                            error: {
                                code: 'SAMPLING_UNAVAILABLE',
                                message: 'MCP Sampling is not available. explain_analysis requires an LLM via Sampling.',
                                retryable: false,
                            },
                        };
                        break;
                    ...
                    try {
                        ...
                        const explanation = typeof samplingResult.content === 'string'
                            ? samplingResult.content
                            : samplingResult.content.type === 'text'
                                ? samplingResult.content.text
                ...
                case 'comprehensive': {
                    ...
                    const shouldUseTask = await this.shouldUseTaskForComprehensive(comprehensiveInput.spreadsheetId, comprehensiveInput.sheetId);
                    if (shouldUseTask && this.context.taskStore) {
                        // Create task for long-running analysis
                        const task = await this.context.taskStore.createTask({ ttl: 3600000 }, // 1 hour TTL
                        ...
                        void this.runComprehensiveAnalysisTask(task.taskId, comprehensiveInput).catch((error) => {
                    ...
                    const analyzer = new ComprehensiveAnalyzer(this.sheetsApi, {
                        includeFormulas: 'includeFormulas' in comprehensiveInput
                            ? comprehensiveInput.includeFormulas
                            : true,
                        includeVisualizations: 'includeVisualizations' in comprehensiveInput
                            ? comprehensiveInput.includeVisualizations
                            : true,
                        includePerformance: 'includePerformance' in comprehensiveInput
                            ? comprehensiveInput.includePerformance
                            : true,
                        forceFullData: 'forceFullData' in comprehensiveInput
                            ? comprehensiveInput.forceFullData
                            : false,
                        samplingThreshold: 'samplingThreshold' in comprehensiveInput
                            ? comprehensiveInput.samplingThreshold
                            : 10000,
                        sampleSize: 'sampleSize' in comprehensiveInput ? comprehensiveInput.sampleSize : 100,
                        sheetId: comprehensiveInput.sheetId,
                        context: comprehensiveInput.context,
                        cursor: 'cursor' in comprehensiveInput ? comprehensiveInput.cursor : undefined,
                        pageSize: 'pageSize' in comprehensiveInput
                            ? comprehensiveInput.pageSize
                            : undefined,
                    });
                    try {
     ...
     */
    async shouldUseTaskForComprehensive(spreadsheetId, sheetId) {
        try {
            ...
            const totalCells = sheets.reduce((sum, s) => sum +
                (s.properties?.gridProperties?.rowCount || 0) *
                    (s.properties?.gridProperties?.columnCount || 0), 0);
            const sheetCount = sheets.length;
            // Use task if:
            // - >20 sheets OR
            // - >1M cells OR
            // - >50K rows in any sheet
            const hasLargeSheet = sheets.some((s) => (s.properties?.gridProperties?.rowCount || 0) > 50000);
     ...
     */
    async runComprehensiveAnalysisTask(taskId, input) {
        const taskStore = this.context.taskStore;
        if (!taskStore) {
            throw new ServiceError('Task store not available', 'SERVICE_NOT_INITIALIZED', 'TaskStore', false);
        }
        try {
            ...
            // Create analyzer
            const analyzer = new ComprehensiveAnalyzer(this.sheetsApi, {
                includeFormulas: 'includeFormulas' in input ? input.includeFormulas : true,
                includeVisualizations: 'includeVisualizations' in input ? input.includeVisualizations : true,
                includePerformance: 'includePerformance' in input ? input.includePerformance : true,
                forceFullData: 'forceFullData' in input ? input.forceFullData : false,
                samplingThreshold: 'samplingThreshold' in input ? input.samplingThreshold : 10000,
                sampleSize: 'sampleSize' in input ? input.sampleSize : 100,
                sheetId: input.sheetId,
                context: input.context,
                cursor: 'cursor' in input ? input.cursor : undefined,
                pageSize: 'pageSize' in input ? input.pageSize : undefined,
            });
            // Run analysis
