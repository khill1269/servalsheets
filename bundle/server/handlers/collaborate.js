...
export class CollaborateHandler extends BaseHandler {
    ...
    async handle(input) {
        ...
        if (req.action.startsWith('share_') && !this.context.auth?.hasElevatedAccess) {
            ...
            const authUrl = validator.generateIncrementalAuthUrl(requirements?.missing ?? ['https://www.googleapis.com/auth/drive']);
        ...
        try {
            ...
            switch (inferredReq.action) {
                // ========== SHARING ACTIONS ==========
                case 'share_add':
                    response = await this.handleShareAdd(inferredReq);
                    break;
                case 'share_update':
                    response = await this.handleShareUpdate(inferredReq);
                    break;
                case 'share_remove':
                    response = await this.handleShareRemove(inferredReq);
                    break;
                case 'share_list':
                    response = await this.handleShareList(inferredReq);
                    break;
                case 'share_get':
                    response = await this.handleShareGet(inferredReq);
                    break;
                case 'share_transfer_ownership':
                    response = await this.handleShareTransferOwnership(inferredReq);
                    break;
                case 'share_set_link':
                    response = await this.handleShareSetLink(inferredReq);
                    break;
                case 'share_get_link':
                    response = await this.handleShareGetLink(inferredReq);
                    break;
                // ========== COMMENT ACTIONS ==========
                case 'comment_add':
                    response = await this.handleCommentAdd(inferredReq);
                    break;
                case 'comment_update':
                    response = await this.handleCommentUpdate(inferredReq);
                    break;
                case 'comment_delete':
                    response = await this.handleCommentDelete(inferredReq);
                    break;
                case 'comment_list':
                    response = await this.handleCommentList(inferredReq);
                    break;
                case 'comment_get':
                    response = await this.handleCommentGet(inferredReq);
                    break;
                case 'comment_resolve':
                    response = await this.handleCommentResolve(inferredReq);
                    break;
                case 'comment_reopen':
                    response = await this.handleCommentReopen(inferredReq);
                    break;
                case 'comment_add_reply':
                    response = await this.handleCommentAddReply(inferredReq);
                    break;
                case 'comment_update_reply':
                    response = await this.handleCommentUpdateReply(inferredReq);
                    break;
                case 'comment_delete_reply':
                    response = await this.handleCommentDeleteReply(inferredReq);
                    break;
                // ========== VERSION ACTIONS ==========
                case 'version_list_revisions':
                    response = await this.handleVersionListRevisions(inferredReq);
                    break;
                case 'version_get_revision':
                    response = await this.handleVersionGetRevision(inferredReq);
                    break;
                case 'version_restore_revision':
                    response = await this.handleVersionRestoreRevision(inferredReq);
                    break;
                case 'version_keep_revision':
                    response = await this.handleVersionKeepRevision(inferredReq);
                    break;
                case 'version_create_snapshot':
                    response = await this.handleVersionCreateSnapshot(inferredReq);
                    break;
                case 'version_list_snapshots':
                    response = await this.handleVersionListSnapshots(inferredReq);
                    break;
                case 'version_restore_snapshot':
                    response = await this.handleVersionRestoreSnapshot(inferredReq);
                    break;
                case 'version_delete_snapshot':
                    response = await this.handleVersionDeleteSnapshot(inferredReq);
                    break;
                case 'version_compare':
                    response = await this.handleVersionCompare(inferredReq);
                    break;
                case 'version_export':
                    response = await this.handleVersionExport(inferredReq);
                    break;
                default:
                    response = this.error({
                        code: 'INVALID_PARAMS',
                        message: `Unknown action: ${inferredReq.action}`,
                        retryable: false,
                    });
            }
    ...
    // SHARING ACTIONS
    // ============================================================
    async handleShareAdd(input) {
        const requestBody = {
            type: input.type,
            role: input.role,
        };
        if (input.emailAddress)
            requestBody.emailAddress = input.emailAddress;
        if (input.domain)
            requestBody.domain = input.domain;
        if (input.expirationTime)
            requestBody.expirationTime = input.expirationTime;
    ...
    async handleShareRemove(input) {
        ...
        if (this.context.elicitationServer) {
            const confirmation = await confirmDestructiveAction(this.context.elicitationServer, 'share_remove', `Remove permission (ID: ${input.permissionId}) from spreadsheet ${input.spreadsheetId}. This will revoke access for the user. This action cannot be undone.`);
    ...
    async handleShareTransferOwnership(input) {
        if (input.safety?.dryRun) {
    ...
    async handleShareSetLink(input) {
        if (!input.enabled) {
    ...
    async handleShareGetLink(input) {
    ...
    async handleCommentUpdate(input) {
        if (input.safety?.dryRun) {
    ...
    async handleCommentDelete(input) {
        if (input.safety?.dryRun) {
            return this.success('comment_delete', {}, undefined, true);
        }
        // Request confirmation if elicitation available
        if (this.context.elicitationServer) {
            const confirmation = await confirmDestructiveAction(this.context.elicitationServer, 'comment_delete', `Delete comment (ID: ${input.commentId}) from spreadsheet ${input.spreadsheetId}. This will permanently remove the comment and all its replies. This action cannot be undone.`);
            if (!confirmation.confirmed) {
                return this.error({
                    code: 'PRECONDITION_FAILED',
                    message: confirmation.reason || 'User cancelled the operation',
    ...
    async handleCommentResolve(input) {
        const response = await this.driveApi.comments.update({
            fileId: input.spreadsheetId,
            commentId: input.commentId,
    ...
    async handleCommentReopen(input) {
        const response = await this.driveApi.comments.update({
            fileId: input.spreadsheetId,
            commentId: input.commentId,
    ...
    async handleCommentAddReply(input) {
    ...
    async handleCommentUpdateReply(input) {
        if (input.safety?.dryRun) {
    ...
    async handleCommentDeleteReply(input) {
        if (input.safety?.dryRun) {
            return this.success('comment_delete_reply', {}, undefined, true);
        }
        // Request confirmation if elicitation available
        if (this.context.elicitationServer) {
            const confirmation = await confirmDestructiveAction(this.context.elicitationServer, 'comment_delete_reply', `Delete reply (ID: ${input.replyId}) from comment ${input.commentId} in spreadsheet ${input.spreadsheetId}. This action cannot be undone.`);
            if (!confirmation.confirmed) {
                return this.error({
                    code: 'PRECONDITION_FAILED',
                    message: confirmation.reason || 'User cancelled the operation',
    ...
    // VERSION ACTIONS
    // ============================================================
    async handleVersionListRevisions(input) {
    ...
    async handleVersionGetRevision(input) {
    ...
    async handleVersionRestoreRevision(input) {
        if (input.safety?.dryRun) {
    ...
    async handleVersionKeepRevision(input) {
    ...
    async handleVersionCreateSnapshot(input) {
    ...
    async handleVersionListSnapshots(input) {
    ...
    async handleVersionRestoreSnapshot(input) {
        if (input.safety?.dryRun) {
    ...
    async handleVersionDeleteSnapshot(input) {
        if (input.safety?.dryRun) {
            return this.success('version_delete_snapshot', {}, undefined, true);
        }
        // Request confirmation if elicitation available
        if (this.context.elicitationServer) {
            const confirmation = await confirmDestructiveAction(this.context.elicitationServer, 'version_delete_snapshot', `Delete Drive snapshot (ID: ${input.snapshotId}). This will permanently remove the snapshot file from Drive. This action cannot be undone.`);
            if (!confirmation.confirmed) {
                return this.error({
                    code: 'PRECONDITION_FAILED',
                    message: confirmation.reason || 'User cancelled the operation',
    ...
    async handleVersionCompare(input) {
        if (!this.driveApi) {
            return this.error({
                code: 'INTERNAL_ERROR',
    ...
    async handleVersionExport(input) {
        ...
        if (input.revisionId && input.revisionId !== 'head') {
            return this.error({
                ...
                message: 'Exporting specific revisions is not supported. Use revisionId="head" or omit it to export the current version.',
                details: {
                    revisionId: input.revisionId,
                    reason: 'Google Drive API does not support exporting historical revisions without restoring them first',
                },
                retryable: false,
                suggestedFix: 'To export a specific revision: (1) Use restore_revision to restore it, (2) Use export_version without revisionId, (3) Optionally restore back to current version.',
            });
        }
        try {
            ...
            if (error.code === 404) {
                return this.error(createNotFoundError({
                    ...
                    searchSuggestion: 'Verify the spreadsheet ID is correct and you have permission to access it',
                }));
            }
            return this.error({
                code: 'INTERNAL_ERROR',
