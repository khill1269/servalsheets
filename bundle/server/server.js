...
import { createServerCapabilities, SERVER_INSTRUCTIONS, TOOL_ICONS, TOOL_EXECUTION_CONFIG, } from './mcp/features-2025-11-25.js';
import { recordToolCall, updateQueueMetrics } from './observability/metrics.js';
import { BatchCompiler, RateLimiter, DiffEngine, PolicyEnforcer, RangeResolver, TaskStoreAdapter, } from './core/index.js';
import { SnapshotService, createGoogleApiClient } from './services/index.js';
...
import { TOOL_DEFINITIONS, createFastToolHandlerMap, buildToolResponse, registerServalSheetsResources, registerServalSheetsPrompts, prepareSchemaForRegistration, } from './mcp/registration.js';
import { recordSpreadsheetId } from './mcp/completions.js';
import { registerKnowledgeResources, registerHistoryResources, registerCacheResources, registerTransactionResources, registerConflictResources, registerImpactResources, registerValidationResources, registerMetricsResources, registerConfirmResources, registerAnalyzeResources, registerReferenceResources, registerGuideResources, registerDecisionResources, registerExamplesResources, registerSheetResources, registerSchemaResources, } from './resources/index.js';
...
export class ServalSheetsServer {
    ...
    registerTools() {
        for (const tool of TOOL_DEFINITIONS) {
            ...
            if (process.env['NODE_ENV'] !== 'production') {
                const isZodSchema = (schema) => Boolean(schema && typeof schema === 'object' && '_def' in schema);
            ...
            if (supportsTasks) {
                ...
                this._server.experimental.tasks.registerToolTask(tool.name, {
    ...
    createToolTaskHandler(toolName) {
        return {
     ...
     */
    async handleTaskCancel(taskId, taskStore) {
        try {
        ...
        return this.requestQueue.add(async () => {
            ...
            return runWithRequestContext(requestContext, async () => {
                ...
                try {
                    ...
                    if (isError) {
                        const errorDetail = response?.success === false ? response.error : result.error;
 ...
 */
export async function createServalSheetsServer(options = {}) {
    ...
    if (isProduction && !redisUrl) {
        throw new Error('Redis is required in production mode. Set REDIS_URL environment variable.\n' +
