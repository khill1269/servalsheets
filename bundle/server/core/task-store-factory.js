 ...
 */
export async function createTaskStore(config = {}) {
    ...
    if (storeType === 'redis') {
        const redisUrl = config.redisUrl ?? process.env['REDIS_URL'];
        if (!redisUrl) {
            throw new Error('Redis task store requested but REDIS_URL not configured. ' +
 ...
 */
export function getRecommendedTaskStoreType() {
    const isProduction = process.env['NODE_ENV'] === 'production';
    const hasRedis = Boolean(process.env['REDIS_URL']);
    if (isProduction && !hasRedis) {
        baseLogger.warn('Production environment detected without Redis. ' +
...
//# sourceMappingURL=task-store-factory.js.map