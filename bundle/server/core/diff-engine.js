...
export class DiffEngine {
     ...
     */
    async captureState(spreadsheetId, options) {
        ...
        const sheets = await Promise.all((response.data.sheets ?? []).map((sheet) => queue.add(async () => {
            ...
            const [sampleData, values] = await Promise.all([
                ...
                (async () => {
                    ...
                    const lastRowsPromise = rowCount > sampleSize * 2
                        ? this.getRangeValues(spreadsheetId, `'${escapedTitle}'!A${rowCount - sampleSize}:ZZ${rowCount}`)
                        : Promise.resolve([]);
                ...
                (async () => {
                    ...
                    const maxRows = Math.min(rowCount, Math.ceil(maxFullDiffCells / Math.max(columnCount, 1)));
     ...
     */
    async captureRangeState(spreadsheetId, range) {
     ...
     * ```
     */
    captureStateFromResponse(spreadsheetId, sheetsResponse, options) {
        const targetTier = options?.tier ?? this.defaultTier;
        const sampleSize = options?.sampleSize ?? this.sampleSize;
        const sheets = (sheetsResponse.sheets ?? [])
            ...
            if (sheet.data && sheet.data.length > 0 && sheet.data[0] && targetTier === 'FULL') {
                ...
                values = (sheet.data[0].rowData ?? []).map((rowData) => (rowData.values ?? []).map((cellData) => cellData.effectiveValue?.numberValue ??
                ...
                const allRows = (sheet.data[0].rowData ?? []).map((rowData) => (rowData.values ?? []).map((cellData) => cellData.effectiveValue?.numberValue ??
                    cellData.effectiveValue?.stringValue ??
                    cellData.effectiveValue?.boolValue ??
                    cellData.formattedValue ??
                    null));
                const firstRows = allRows.slice(0, sampleSize);
                const lastRows = rowCount > sampleSize * 2 ? allRows.slice(Math.max(0, rowCount - sampleSize)) : [];
     ...
     * ```
     */
    captureRangeStateFromResponse(range, updatedData) {
        const values = (updatedData?.values ?? []);
        const valuesString = JSON.stringify(values);
        return {
            checksum: createHash('md5').update(valuesString).digest('hex'),
            rowCount: values.length,
     ...
     */
    async diff(before, after, options) {
     ...
     */
    async sampleDiff(before, after, sampleSize = this.sampleSize) {
        const samples = {
            firstRows: [],
            lastRows: [],
            randomRows: [],
        ...
        for (const sheet of after.sheets) {
            ...
            const afterFirst = sheet.sampleData?.firstRows ??
            ...
            this.collectSampleChanges(sheet.title, afterFirst, beforeFirst, 0, samples.firstRows, changedRows);
            const shouldCheckLast = sheet.rowCount > sampleSize * 2;
            if (shouldCheckLast) {
                const afterLast = sheet.sampleData?.lastRows ??
                    (await this.getRangeValues(after.spreadsheetId, `'${escapedTitle}'!A${sheet.rowCount - sampleSize}:ZZ${sheet.rowCount}`));
                ...
                this.collectSampleChanges(sheet.title, afterLast, beforeLast, startRowIndex, samples.lastRows, changedRows);
            }
        }
        return {
            tier: 'SAMPLE',
    ...
    async fullDiff(before, after) {
        ...
        const sheetResults = await Promise.all(after.sheets.map((afterSheet) => queue.add(async () => {
            ...
            const afterValues = await this.ensureValues(afterSheet, after.spreadsheetId, maxCells - cellsCompared);
            ...
            const changedBlocks = this.identifyChangedBlocks(beforeSheet?.blockChecksums, afterSheet.blockChecksums);
            return this.diffSheetValues(afterSheet.title, afterValues, beforeValues, changedBlocks, maxCells - cellsCompared);
        })));
    ...
    formatCell(sheetTitle, colIndex, rowIndex) {
     ...
     */
    selectTier(before, after, options) {
     ...
     */
    estimateChangedCells(before, after) {
    ...
    async getRangeValues(spreadsheetId, range) {
        try {
            const response = await this.sheetsApi.spreadsheets.values.get({
                spreadsheetId,
                range,
    ...
    collectSampleChanges(sheetTitle, afterValues, beforeValues, rowOffset, bucket, changedRows) {
    ...
    async ensureValues(sheet, spreadsheetId, remainingBudget) {
        if (sheet.values) {
            return sheet.values;
        }
     ...
     */
    identifyChangedBlocks(beforeChecksums, afterChecksums) {
     ...
     */
    diffSheetValues(sheetTitle, afterValues, beforeValues, changedBlocks, maxCells) {
        const changes = [];
        let cellsCompared = 0;
        let cellsAdded = 0;
        let cellsRemoved = 0;
