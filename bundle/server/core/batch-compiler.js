...
import { monitorPayload } from '../utils/payload-monitor.js';
...
export class BatchCompiler {
    ...
    async execute(batch, safety) {
        ...
        try {
            ...
            const payloadMetrics = monitorPayload(`batchUpdate:${batch.spreadsheetId}`, requestPayload, response.data);
     ...
     */
    async executeWithSafety(options) {
     ...
     */
    async executeAll(batches, safety) {
        ...
        const spreadsheetResults = await Promise.all(Array.from(batchesBySpreadsheet.values()).map(async (spreadsheetBatches) => {
            const groupResults = [];
     ...
     */
    groupBySpreadsheet(wrappedRequests) {
        return wrappedRequests.reduce((acc, wrapped) => {
     ...
     */
    convertIntentsToWrappedRequests(intents) {
        return intents.map((intent) => {
            const requestType = INTENT_TO_REQUEST_TYPE[intent.type];
            return {
                request: { [requestType]: intent.payload },
                metadata: {
...
//# sourceMappingURL=batch-compiler.js.map