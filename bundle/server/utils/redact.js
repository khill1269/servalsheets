 ...
 */
export function redactObject(obj, seen = new WeakSet(), depth = 0) {
    ...
    if (Array.isArray(obj)) {
        return obj.map((item) => redactObject(item, seen, depth + 1));
    }
    // Handle Error objects specially (preserve stack trace)
    if (obj instanceof Error) {
        const redacted = new obj.constructor(redactString(obj.message));
        ...
        for (const [key, value] of Object.entries(obj)) {
            if (key !== 'message' && key !== 'stack' && key !== 'name') {
                redacted[key] = redactObject(value, seen, depth + 1);
            }
        }
        return redacted;
    }
 ...
 */
export function redactAll(...values) {
    return values.map((v) => redact(v));
}
