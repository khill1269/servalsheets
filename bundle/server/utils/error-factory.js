...
export function createNotFoundError(params) {
    ...
    const resolutionSteps = [`1. Verify the ${resourceType} ID is correct: ${resourceId}`];
    if (resourceType === 'sheet') {
        resolutionSteps.push(`2. List all sheets in the spreadsheet: Use 'sheets_core' tool with action 'list_sheets'`, `3. Check if the sheet name has changed`, `4. Verify the sheet hasn't been deleted`);
    }
    else if (resourceType === 'spreadsheet') {
        resolutionSteps.push(`2. Verify you have access to the spreadsheet`, `3. Check if the spreadsheet was deleted or moved to trash`, `4. Confirm the URL is correct: https://docs.google.com/spreadsheets/d/${resourceId}`);
    }
    else if (resourceType === 'range') {
        resolutionSteps.push(`2. Verify the A1 notation is valid (e.g., "Sheet1!A1:B10")`, `3. Check if the sheet name exists`, `4. Ensure the range coordinates are within sheet bounds`);
    }
    else if (resourceType === 'operation') {
        resolutionSteps.push(`2. List available operations using sheets_history tool with action 'list'`, `3. Check the operation ID is correct`, `4. Operations may have been cleared or expired from history`);
    }
    else if (resourceType === 'snapshot') {
        resolutionSteps.push(`2. Verify the operation was created with snapshot enabled`, `3. Check snapshot storage configuration`, `4. Some operations may not support snapshots`);
    }
    if (searchSuggestion) {
    ...
    const errorCode = resourceType === 'sheet'
        ? 'SHEET_NOT_FOUND'
        : resourceType === 'range'
            ? 'RANGE_NOT_FOUND'
...
export function createAuthenticationError(params) {
    ...
    switch (reason) {
        case 'missing_token':
            ...
            resolutionSteps.push('1. Run authentication: npm run auth', '2. Follow the OAuth flow in your browser', '3. Grant required permissions when prompted', '4. Retry the operation after authentication completes');
            break;
        case 'invalid_token':
            message = 'Authentication failed: Invalid access token';
            resolution = 'Re-authenticate to obtain a new valid token';
            resolutionSteps.push('1. Clear existing token storage', '2. Run authentication: npm run auth', '3. Complete the OAuth flow', '4. Retry the operation');
            break;
        case 'expired_token':
            message = 'Authentication failed: Access token expired';
            resolution = 'Refresh the access token or re-authenticate';
            resolutionSteps.push('1. Token refresh should happen automatically', '2. If refresh fails, re-authenticate: npm run auth', '3. Retry the operation');
            break;
        case 'insufficient_scopes':
            ...
            resolutionSteps.push('1. Run authentication with force consent: npm run auth', '2. Grant all requested permissions (especially Google Sheets scope)', '3. Ensure you select all checkboxes in the OAuth consent screen', '4. Retry the operation');
            break;
    }
    return {
        code: 'PERMISSION_DENIED',
...
export function createValidationError(params) {
    ...
    const resolutionSteps = [`1. Check the '${field}' parameter value`];
    if (expectedFormat) {
    ...
    resolutionSteps.push(`5. Review the tool schema for '${field}' parameter requirements`, `6. Correct the value and retry`);
 ...
 */
export function enrichErrorWithContext(error, context) {
...
export function detectErrorPattern(errors) {
    ...
    for (const error of errors) {
        ...
        const timestamp = error.details?.['timestamp'] || Date.now();
    ...
    const permissionErrors = errors.filter((e) => e.code === 'PERMISSION_DENIED' || e.code === 'FORBIDDEN').length;
    const authTokenErrors = errors.filter((e) => e.category === 'auth' &&
        e.code !== 'PERMISSION_DENIED' &&
        e.code !== 'FORBIDDEN' &&
        (e.message?.includes('token') || e.message?.includes('auth'))).length;
    // Rate limit pattern: Multiple quota errors in short time
    if (quotaErrors >= 3) {
        return {
            pattern: 'rate_limit',
            frequency: quotaErrors,
            suggestedAction: 'Implement exponential backoff and reduce request frequency. Consider batch operations.',
    ...
    if (permissionErrors >= 2) {
        return {
            ...
            suggestedAction: 'Verify user has required permissions. Use sheets_collaborate tool to check access levels.',
            affectedOperations: Array.from(operationSet),
            firstSeen,
            lastSeen,
        };
    ...
    if (authTokenErrors >= 2) {
        return {
            ...
            suggestedAction: 'Check token refresh logic. Ensure OAuth tokens are being refreshed before expiry.',
            affectedOperations: Array.from(operationSet),
            firstSeen,
            lastSeen,
        };
