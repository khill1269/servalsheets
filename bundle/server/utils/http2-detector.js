 ...
 */
export function logHTTP2Capabilities() {
 ...
 */
export function detectHTTPVersion(response) {
    ...
    if (response && typeof response === 'object') {
        ...
        if (resp['config'] &&
            ...
            resp['config']['httpVersion']) {
            return String(resp['config']['httpVersion']);
        }
        // Check response.request.protocol (Node.js http/https)
        if (resp['request'] &&
            typeof resp['request'] === 'object' &&
            resp['request']['protocol']) {
            return String(resp['request']['protocol']);
        }
 ...
 */
export function logHTTPVersion(response, operation) {
    // Only log in development or when HTTP_DEBUG is enabled
    const shouldLog = process.env['NODE_ENV'] === 'development' || process.env['HTTP_DEBUG'] === 'true';
    if (!shouldLog) {
        return;
    }
 ...
 */
export function validateHTTP2Config(http2Enabled) {
    const warnings = [];
    const versionInfo = getNodeVersionInfo();
...
//# sourceMappingURL=http2-detector.js.map