...
export class CacheManager {
    cache;
    cleanupTimer;
    rangeDependencies; // spreadsheetId:range -> cache keys
    requestMerger;
    // Configuration
    enabled;
    defaultTTL;
     ...
     */
    invalidatePattern(pattern, namespace) {
     ...
     */
    getExpiringEntries(thresholdMs, namespace) {
        const now = Date.now();
     ...
     */
    trackRangeDependency(spreadsheetId, range, cacheKey) {
     ...
     */
    invalidateRange(spreadsheetId, range) {
     ...
     */
    findOverlappingRanges(spreadsheetId, range) {
        const overlapping = [];
        ...
        for (const depKey of this.rangeDependencies.keys()) {
            ...
            if (depSpreadsheetId !== spreadsheetId)
                continue;
 ...
 */
export function createCacheKey(operation, params) {
