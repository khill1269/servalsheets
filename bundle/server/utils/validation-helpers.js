 ...
 */
export function assertRegexGroups(match, requiredGroups, context) {
    if (!match) {
...
export function parseA1Range(range) {
    ...
    if (startRow > endRow) {
        throw new ValidationError(`Start row (${startRow}) cannot be greater than end row (${endRow})`, 'range');
    }
    return {
        startCol,
        startRow,
 ...
 */
export function parseA1Cell(cell) {
 ...
 */
export function isValidSpreadsheetId(value) {
 ...
 */
export function safeArrayAccess(array, index, name) {
    if (index < 0 || index >= array.length) {
        throw new ValidationError(`${name}: Index ${index} out of bounds (array length: ${array.length})`, name);
    }
