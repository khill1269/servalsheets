 ...
 */
export function analyzeBatchEfficiency(intents) {
    ...
    const potentialSavings = spreadsheetCount > 1 ? Math.max(0, spreadsheetCount - Math.ceil(intentCount / 10)) : 0;
    const metrics = {
        intentCount,
        spreadsheetCount,
 ...
 */
export function clearBatchEfficiencyHistory() {
    efficiencyHistory.length = 0;
}
/**
 * Suggest batch optimization strategies
 */
export function suggestBatchOptimizations(intents) {
    const suggestions = [];
...
//# sourceMappingURL=batch-efficiency.js.map