...
export class CircuitBreakerError extends Error {
    circuitName;
    nextAttemptTime;
    constructor(message, circuitName, nextAttemptTime) {
        super(message);
...
export class CircuitBreaker {
    ...
    async execute(operation, fallback) {
        ...
        if (this.state === 'open') {
            if (Date.now() < this.nextAttemptTime) {
                ...
                if (this.fallbackStrategies.length > 0) {
                    ...
                    return (await this.executeFallbacks(error));
                }
                ...
                throw new CircuitBreakerError(`Circuit breaker [${this.name}] is OPEN`, this.name, this.nextAttemptTime);
            }
        ...
        try {
            ...
            if (this.fallbackStrategies.length > 0) {
                return (await this.executeFallbacks(error instanceof Error ? error : new Error(String(error))));
            }
...
export const FallbackStrategies = {
     ...
     * );
     */
    cachedData: (cache, key, priority = 100) => ({
        name: 'cached-data',
        priority,
        execute: async () => {
     ...
     * );
     */
    degradedMode: (degradedData, priority = 50) => ({
        name: 'degraded-mode',
        priority,
        execute: async () => {
     ...
     * );
     */
    safeDefault: (defaultValue, priority = 10) => ({
        name: 'safe-default',
        priority,
        execute: async () => {
     ...
     * );
     */
    retryWithBackoff: (operation, maxRetries = 3, baseDelayMs = 1000, priority = 80) => ({
     ...
     * );
     */
    alternateSource: (alternateOperation, priority = 90) => ({
     ...
     * );
     */
    readOnlyMode: (readOnlyResponse, priority = 30) => ({
        name: 'read-only-mode',
        priority,
        execute: async () => {
