 ...
 */
export function generateSafetyWarnings(context, safetyOptions) {
    const warnings = [];
 ...
 */
export async function createSnapshotIfNeeded(snapshotService, context, safetyOptions) {
    ...
    try {
        const snapshot = await snapshotService.create(context.spreadsheetId, `Before ${context.operationType}`);
 ...
 */
export function calculateAffectedCells(range) {
    if (!range)
        return 0;
 ...
 */
export function calculateAffectedRows(startIndex, count) {
    return count;
}
/**
 * Format safety warnings for response
 */
export function formatSafetyWarnings(warnings) {
    return warnings.map((w) => `${w.message}. ${w.suggestion}`);
}
/**
 * Check if dry-run mode should return preview
 */
export function shouldReturnPreview(safetyOptions) {
    return safetyOptions?.dryRun === true;
}
/**
 * Build snapshot info for response
 */
export function buildSnapshotInfo(snapshot) {
...
//# sourceMappingURL=safety-helpers.js.map