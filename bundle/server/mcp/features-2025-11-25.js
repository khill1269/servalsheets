...
## 📋 DEFERRED SCHEMAS (Important!)

Tool schemas are loaded on-demand to optimize performance. **Before calling a tool for the first time:**

1. Read schema://tools to see all available tools
2. Read schema://tools/{toolName} to get the full schema for that tool
   Example: Read schema://tools/sheets_data to see all data operations

This ensures you know the exact parameters and available actions for each tool.

**Common schemas to fetch:**
- schema://tools/sheets_auth - Authentication (4 actions)
- schema://tools/sheets_core - Spreadsheet/sheet management (15 actions)
- schema://tools/sheets_data - Read/write cell values (20 actions)
- schema://tools/sheets_format - Cell formatting (18 actions)
`;
    const quickStartInstructions = `
## Quick Start (after auth confirmed)
Use sheets_core action:"get" to explore any spreadsheet.
Use sheets_analyze action:"analyze_structure" for quick overview.
Use sheets_analyze action:"analyze_quality" for data quality assessment.
Use sheets_analyze action:"detect_patterns" for AI-powered pattern detection (via MCP Sampling).
...
- Collaboration: sheets_collaborate
- Advanced: sheets_advanced
- Quality & Safety: sheets_quality, sheets_history, sheets_transaction
- Analysis (Sampling): sheets_analyze
- Composite: sheets_composite
- Fix: sheets_fix
- Session & Confirm: sheets_session, sheets_confirm
- Enterprise: sheets_templates, sheets_bigquery, sheets_appsscript
...
`;
    ...
    if (DEFER_SCHEMAS) {
        return (baseInstructions + deferredSchemaInstructions + quickStartInstructions).trim();
    }
    return (baseInstructions + quickStartInstructions).trim();
}
/**
...
//# sourceMappingURL=features-2025-11-25.js.map