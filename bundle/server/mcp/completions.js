...
class SpreadsheetCache {
    ...
    add(spreadsheetId, title) {
        ...
        if (this.recentIds.size > this.maxSize) {
            const entries = Array.from(this.recentIds.entries()).sort((a, b) => b[1].lastAccess - a[1].lastAccess);
    ...
    getCompletions(partial) {
        ...
        return Array.from(this.recentIds.entries())
            .filter(([id, meta]) => id.toLowerCase().includes(lower) || meta.title.toLowerCase().includes(lower))
 ...
 */
export function completeAction(toolName, partial) {
 ...
 */
export function completeSpreadsheetId(partial) {
 ...
 */
export function completeRange(partial) {
    const lower = partial.toLowerCase();
...
export function extractSpreadsheetId(input) {
    ...
    if (request && typeof request === 'object') {
        const nested = request['spreadsheetId'];
 ...
 */
export function recordSpreadsheetId(input) {
 ...
 */
export function completeChartType(partial) {
    const lower = partial.toLowerCase();
 ...
 */
export function completeNumberFormatType(partial) {
    const lower = partial.toLowerCase();
 ...
 */
export function completeConditionType(partial) {
    const lower = partial.toLowerCase();
 ...
 */
export function completeFormatPreset(partial) {
    const lower = partial.toLowerCase();
 ...
 */
export function completePermissionRole(partial) {
    const lower = partial.toLowerCase();
 ...
 */
export function createCompletionResult(values) {
    return {
        completion: {
