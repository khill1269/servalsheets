 ...
 */
import { z } from 'zod';
import { FirstOperationPromptArgsSchema, AnalyzeSpreadsheetPromptArgsSchema, TransformDataPromptArgsSchema, CreateReportPromptArgsSchema, CleanDataPromptArgsSchema, MigrateDataPromptArgsSchema, SetupBudgetPromptArgsSchema, ImportDataPromptArgsSchema, SetupCollaborationPromptArgsSchema, DiagnoseErrorsPromptArgsSchema, SafeOperationPromptArgsSchema, BulkImportPromptArgsSchema, UndoChangesPromptArgsSchema, } from '../../schemas/prompts.js';
// ============================================================================
 ...
 */
export function registerServalSheetsPrompts(server) {
    // === ONBOARDING PROMPTS ===
...
What would you like to do first?`,
    ...
    server.registerPrompt('test_connection', {
...
If auth fails, follow the authentication flow first.`,
    ...
    server.registerPrompt('first_operation', {
...
Safety tips: Always read before modify, use dryRun for destructive ops.`,
    ...
    // === ANALYSIS PROMPTS ===
    server.registerPrompt('analyze_spreadsheet', {
...
Provide: quality score, issues found, recommended fixes.`,
    ...
    server.registerPrompt('transform_data', {
...
5. Verify results`,
    ...
    server.registerPrompt('create_report', {
...
Final: Auto-resize, freeze header, add timestamp`,
    ...
    server.registerPrompt('clean_data', {
...
5. Validate: Compare before/after`,
    ...
    server.registerPrompt('migrate_data', {
...
Safety: Creates snapshots of both sheets before migration.`,
    ...
    server.registerPrompt('setup_budget', {
...
Final: Apply professional formatting, add instructions sheet.`,
    ...
    server.registerPrompt('import_data', {
...
Pro tip: Use sheets_transaction to batch all operations into 1 API call.`,
    ...
    server.registerPrompt('setup_collaboration', {
...
- Reserve "owner" role transfers for handoffs`,
    ...
    server.registerPrompt('diagnose_errors', {
...
- Preventive measures`,
    ...
    server.registerPrompt('recover_from_error', {
        ...
        const errorCode = args['errorCode'] || 'UNKNOWN_ERROR';
...
• No access permission`,
        ...
        const recovery = recoveryGuide[errorCode] ||
            `🔧 ${errorCode} Recovery
...
• Restart: Quit Claude Desktop (Cmd+Q), wait 5s, relaunch`;
    ...
    server.registerPrompt('troubleshoot_performance', {
        ...
        const responseTime = args['responseTime'] || 0;
        return {
            messages: [
                {
                    role: 'user',
...
Apply fixes and retest!`,
    ...
    server.registerPrompt('fix_data_quality', {
        ...
        const issues = args['issues'] || 'auto-detect';
        return {
            messages: [
                {
                    role: 'user',
...
• Document changes: sheets_collaborate action="comment_add"`,
    ...
    server.registerPrompt('optimize_formulas', {
        ...
        const spreadsheetId = args['spreadsheetId'];
        const range = args['range'] || 'entire sheet';
        return {
            messages: [
                {
                    role: 'user',
...
• Monitor performance over time`,
    ...
    server.registerPrompt('bulk_import_data', {
        ...
        const spreadsheetId = args['spreadsheetId'];
        const dataSize = args['dataSize'] || 0;
        const dataSource = args['dataSource'] || 'external source';
        return {
            messages: [
                {
                    role: 'user',
...
Import complete! ✅`,
    ...
    server.registerPrompt('safe_operation', {
        description: '🛡️ Execute destructive operations safely with dry-run → confirm → execute workflow',
        argsSchema: SafeOperationPromptArgsSchema,
    }, async (args) => {
        const operationType = args['operationType'];
        const affectedRange = args['affectedRange'] || 'auto-detect';
        return {
            messages: [
                {
                    role: 'user',
...
Remember: DRY-RUN → IMPACT → CONFIRM → SNAPSHOT → EXECUTE → VERIFY`,
    ...
    server.registerPrompt('bulk_import', {
        ...
        const targetSheet = args['targetSheet'] || 'new sheet';
        return {
            messages: [
                {
                    role: 'user',
...
Then you'll always have an easy undo path!`,
    ...
    server.registerPrompt('when_to_confirm', {
...
8. Offer dryRun when user seems unsure`,
    ...
    server.registerPrompt('confirmation_examples', {
...
6. When in doubt → Ask nicely`,
    ...
    server.registerPrompt('advanced_data_migration', {
        description: '🚀 Advanced multi-sheet, multi-spreadsheet data migration with transformation and validation',
        argsSchema: {
...
Ready to execute migration! 🚀`,
    ...
    server.registerPrompt('performance_audit', {
        ...
        const focusAreas = args['focusAreas'] || ['all'];
        return {
            messages: [
                {
                    role: 'user',
...
Audit complete! Review findings and implement recommendations. 🎯`,
    ...
    server.registerPrompt('batch_optimizer', {
        ...
        const operationType = args['operationType'];
        const operationCount = args['operationCount'] || 10;
        return {
            messages: [
                {
                    role: 'user',
...
Ready to optimize! Convert your operations now. 🚀`,
    ...
    server.registerPrompt('ultimate_analysis', {
...
Ready to analyze! What insights do you need? 🚀`,
    ...
    server.registerPrompt('create_visualization', {
...
Ready to visualize your data! 🎨`,
    ...
    server.registerPrompt('analyze_with_history', {
