...
DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE, } from '../config/constants.js';
...
export class ComprehensiveAnalyzer {
    ...
    async analyze(spreadsheetId) {
        ...
        let sheetsToAnalyze = this.config.sheetId !== undefined
        ...
        for (const sheet of paginatedSheets) {
            ...
            const needsSampling = sheet.rowCount > this.config.samplingThreshold && !this.config.forceFullData;
            ...
            if (needsSampling || sheet.rowCount > MAX_ROWS_PER_SHEET) {
                ...
                const sampleResult = await this.tieredRetrieval.getSample(spreadsheetId, sheet.sheetId, sampleSize);
            ...
            // Analyze the sheet
            const analysis = await this.analyzeSheet(spreadsheetId, sheet, data, needsSampling ? sheet.rowCount : data.length);
        ...
        const { summary, topInsights } = this.generateSummary(metadata, sheetAnalyses, aggregate, performance);
        ...
        const estimatedSizeBytes = totalDataRows * 50 + // Row count metadata
     ...
     */
    async analyzeSheet(spreadsheetId, sheet, data, actualRowCount) {
        if (data.length === 0) {
            return {
        ...
        // Check data quality
        const { qualityScore, completeness, consistency, issues } = this.analyzeQuality(headers, dataRows, columns);
        // Detect patterns
    ...
    analyzeColumns(headers, dataRows) {
        return headers.map((header, index) => {
            ...
            const dataType = detectedType === 'email' || detectedType === 'url' || detectedType === 'unknown'
                ? 'text'
                : detectedType;
            const stats = {
            ...
            if (dataType === 'number') {
                ...
                if (numericValues.length > 0) {
                    ...
                    // Standard deviation
                    const variance = numericValues.reduce((sum, val) => sum + Math.pow(val - stats.mean, 2), 0) /
     ...
     */
    analyzeQuality(headers, dataRows, columns) {
        const issues = [];
        ...
        const headerCounts = headers.reduce((acc, h) => {
            acc[h] = (acc[h] || 0) + 1;
            return acc;
        }, {});
        ...
        const qualityScore = Math.max(0, Math.min(100, completeness * 0.5 + Math.max(0, consistency) * 0.5));
        return {
            qualityScore,
            completeness,
     ...
     */
    detectTrends(headers, dataRows, columns) {
        const trends = [];
        columns.forEach((col, index) => {
            if (col.dataType !== 'number' || col.count < 5)
                return;
     ...
     */
    detectAnomaliesEnhanced(headers, dataRows, columns) {
        const anomalies = [];
        columns.forEach((col, colIndex) => {
            if (col.dataType !== 'number' || !col.mean || !col.stdDev)
                return;
     ...
     */
    detectCorrelations(headers, dataRows, columns) {
        const correlations = [];
     ...
     */
    async enrichWithFormulas(spreadsheetId, sheetAnalyses) {
        try {
            const response = await this.sheetsApi.spreadsheets.get({
                spreadsheetId,
            ...
            for (const analysis of sheetAnalyses) {
                const sheetData = response.data.sheets?.find((s) => s.properties?.sheetId === analysis.sheetId);
     ...
     */
    detectVolatileFunctions(formula) {
    ...
    calculateFormulaComplexity(formula) {
        ...
        if (functionCount > 10 || nestedLevel > 4 || length > 200)
            return 'very_complex';
        if (functionCount > 5 || nestedLevel > 2 || length > 100)
            return 'complex';
        if (functionCount > 2 || nestedLevel > 1 || length > 50)
            return 'moderate';
        return 'simple';
    }
    /**
     ...
     */
    generateVisualizationRecommendations(spreadsheetId, analysis) {
     ...
     * Analyze performance
     */
    analyzePerformance(metadata, sheetAnalyses, _aggregate) {
        const recommendations = [];
        let penalties = 0;
        // Check total size
     ...
     */
    calculateAggregates(sheetAnalyses) {
        return {
            ...
            overallQualityScore: sheetAnalyses.length > 0
                ? sheetAnalyses.reduce((sum, s) => sum + s.qualityScore, 0) / sheetAnalyses.length
                : 0,
            overallCompleteness: sheetAnalyses.length > 0
     ...
     */
    generateSummary(metadata, sheetAnalyses, aggregate, performance) {
        const insights = [];
        // Quality insight
        if (aggregate.overallQualityScore >= 80) {
            insights.push(`✅ High data quality (${aggregate.overallQualityScore.toFixed(0)}%) - data is clean and consistent`);
        }
        else if (aggregate.overallQualityScore >= 60) {
            insights.push(`⚠️ Moderate data quality (${aggregate.overallQualityScore.toFixed(0)}%) - ${aggregate.totalIssues} issues found`);
        }
        else {
            insights.push(`❌ Low data quality (${aggregate.overallQualityScore.toFixed(0)}%) - ${aggregate.totalIssues} issues need attention`);
        }
        // Trends insight
        ...
        if (aggregate.totalAnomalies > 0) {
            insights.push(`🔍 ${aggregate.totalAnomalies} anomalies detected - review for data entry errors`);
        }
        // Correlations insight
        if (aggregate.totalCorrelations > 0) {
            const strongCorr = sheetAnalyses.flatMap((s) => s.correlations.filter((c) => c.strength === 'strong' || c.strength === 'very_strong'));
            if (strongCorr.length > 0) {
                insights.push(`🔗 ${strongCorr.length} strong correlation(s) found - ${strongCorr[0]?.columns.join(' ↔ ')}`);
            }
        }
        // Performance insight
        if (performance && performance.score < 70) {
            insights.push(`⚡ Performance score: ${performance.score}% - ${performance.recommendations.length} optimization(s) recommended`);
        }
        // Formula insight
        if (aggregate.totalFormulas > 0) {
            const issueFormulas = sheetAnalyses.reduce((sum, s) => sum + (s.formulas?.issues.length ?? 0), 0);
            if (issueFormulas > 0) {
                insights.push(`📊 ${aggregate.totalFormulas} formulas, ${issueFormulas} need optimization`);
            }
        }
        const summary = `Analyzed "${metadata.title}" with ${metadata.sheets.length} sheet(s), ` +
