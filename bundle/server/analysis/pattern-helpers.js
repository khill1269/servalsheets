 ...
 */
export function detectDistribution(values) {
    if (values.length < 10) {
        return {
            type: 'unknown',
            confidence: 0,
 ...
 */
export function detectChangePoints(values, threshold = 3) {
    if (values.length < 10)
        return [];
    const changePoints = [];
    const mean = values.reduce((sum, v) => sum + v, 0) / values.length;
 ...
 */
export function detectClusters(values, k = 3) {
    if (values.length < k || k < 2)
        return [];
    // Initialize centroids using k-means++
    const centroids = [values[Math.floor(Math.random() * values.length)]];
 ...
 */
export function fitTrendline(x, y, type = 'linear', degree = 2) {
    if (x.length !== y.length || x.length < 3) {
