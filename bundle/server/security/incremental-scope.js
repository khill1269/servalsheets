 ...
 */
export var ScopeCategory;
(function (ScopeCategory) {
    /** Basic spreadsheet operations */
    ScopeCategory["SPREADSHEET"] = "spreadsheet";
    /** File-level Drive operations (create, open) */
    ScopeCategory["DRIVE_FILE"] = "drive_file";
    /** Full Drive operations (share, list all, permissions) */
    ScopeCategory["DRIVE_FULL"] = "drive_full";
    /** Read-only operations */
    ScopeCategory["READONLY"] = "readonly";
...
export class IncrementalScopeRequiredError extends Error {
    ...
    constructor(options) {
        ...
        super(`Operation "${options.operation}" requires additional permissions. ` +
    ...
    toToolResponse() {
        return {
            ...
            structuredContent: {
                ...
                instructions: 'To complete this operation, the user needs to grant additional permissions. ' +
...
export class ScopeValidator {
     ...
     */
    hasRequiredScopes(operation) {
        const opConfig = OPERATION_SCOPES[operation];
        if (!opConfig) {
            // Unknown operation - allow by default
            return true;
        }
        return opConfig.required.every((scope) => this.currentScopes.includes(scope) ||
     ...
     */
    getMissingScopes(operation) {
        const opConfig = OPERATION_SCOPES[operation];
        if (!opConfig) {
            return [];
        }
        return opConfig.required.filter((scope) => !this.currentScopes.includes(scope) && !this.hasScopeUpgrade(scope));
    }
    /**
     * Generate authorization URL for incremental consent
     */
    generateIncrementalAuthUrl(additionalScopes) {
        if (!this.oauthClient) {
     ...
     */
    static getRecommendedScopes(operations) {
 ...
 */
export function requireScopes(operation, validator) {
    validator.validateOperation(operation);
}
/**
 * Check if error is an incremental scope error
 */
export function isIncrementalScopeError(error) {
