 ...
 */
import { z } from 'zod';
import { SpreadsheetIdSchema, SheetIdSchema, SheetInfoSchema, SpreadsheetInfoSchema, ErrorDetailSchema, SafetyOptionsSchema, MutationSummarySchema, ColorSchema, ResponseMetaSchema, } from './shared.js';
// ============================================================================
// Common Schemas
// ============================================================================
const CommonFieldsSchema = z.object({
    verbosity: z
        ...
        .optional()
        .default('standard')
        .describe('Response detail level: minimal (summary only, 80% less tokens), standard (balanced), detailed (full metadata)'),
});
...
const CreateActionSchema = CommonFieldsSchema.extend({
    ...
    timeZone: z
        .string()
        .regex(/^[A-Za-z_]+\/[A-Za-z_]+$/, 'Invalid timezone format (expected: America/New_York, Europe/London, etc.)')
        .optional()
...
const GetUrlActionSchema = CommonFieldsSchema.extend({
    ...
    spreadsheetId: SpreadsheetIdSchema.describe('Spreadsheet ID from URL'),
    sheetId: SheetIdSchema.optional().describe('Optional numeric sheet ID to link to a specific tab (e.g., #gid=0)'),
});
 ...
 */
export const SHEETS_CORE_ANNOTATIONS = {
