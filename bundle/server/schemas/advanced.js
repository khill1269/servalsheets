 ...
 */
import { z } from 'zod';
import { SpreadsheetIdSchema, SheetIdSchema, RangeInputSchema, GridRangeSchema, ColorSchema, ErrorDetailSchema, SafetyOptionsSchema, MutationSummarySchema, ResponseMetaSchema, } from './shared.js';
...
const NamedRangeSchema = z.object({
    ...
    name: z
        .string()
        .max(NAMED_RANGE_NAME_MAX_LENGTH, `Named range name exceeds Google Sheets limit of ${NAMED_RANGE_NAME_MAX_LENGTH} characters`),
...
const AddNamedRangeActionSchema = CommonFieldsSchema.extend({
    ...
    name: z
        ...
        .max(NAMED_RANGE_NAME_MAX_LENGTH, `Named range name exceeds Google Sheets limit of ${NAMED_RANGE_NAME_MAX_LENGTH} characters`)
        .regex(/^[A-Za-z_]\w*$/, 'Must start with letter/underscore, contain only alphanumeric and underscores')
...
export const SHEETS_ADVANCED_ANNOTATIONS = {
