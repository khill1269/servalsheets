...
export class HistoryService {
    ...
    undoStacks = new Map(); // spreadsheetId -> [operationIds]
    redoStacks = new Map(); // spreadsheetId -> [operationIds]
    constructor(options = {}) {
     ...
     */
    getBySpreadsheet(spreadsheetId, count) {
     ...
     */
    markAsUndone(operationId, spreadsheetId) {
     ...
     */
    markAsRedone(operationId, spreadsheetId) {
        const undoStack = this.undoStacks.get(spreadsheetId) || [];
        const redoStack = this.redoStacks.get(spreadsheetId) || [];
     ...
     */
    clearForSpreadsheet(spreadsheetId) {
        // Remove operations
     ...
     */
    getHistory(spreadsheetId, options) {
     ...
     */
    getOperation(spreadsheetId, operationId) {
     ...
     */
    searchHistory(spreadsheetId, filters) {
        let ops = this.operations.filter((op) => op.spreadsheetId === spreadsheetId);
        if (filters.tool) {
 ...
 * @internal
 */
export function resetHistoryService() {
