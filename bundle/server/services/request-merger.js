...
export class RequestMerger {
     ...
     */
    async mergeRead(sheetsApi, spreadsheetId, range, options = {}) {
     ...
     */
    async flushGroup(sheetsApi, spreadsheetId) {
     ...
     */
    async processSheetGroup(sheetsApi, spreadsheetId, sheetKey, requests) {
     ...
     */
    async executeMergedRequest(sheetsApi, spreadsheetId, requests) {
        try {
     ...
     */
    getStats() {
        const avgWindowSize = this.stats.windowSizes.length > 0
            ? this.stats.windowSizes.reduce((a, b) => a + b, 0) / this.stats.windowSizes.length
            : 0;
        const savingsRate = this.stats.totalRequests > 0
...
export function parseA1Range(range) {
    ...
    if (rangeRef) {
        ...
        const rangePattern = /^([A-Z]+)?(\d+)?(?::([A-Z]+)?(\d+)?)?$|^([A-Z]+):([A-Z]+)$|^(\d+):(\d+)$/i;
...
export function rangesOverlap(range1, range2) {
    ...
    // Check row overlap
    const rowOverlap = Math.max(range1.startRow, range2.startRow) <=
        Math.min(range1.endRow || Number.MAX_SAFE_INTEGER, range2.endRow || Number.MAX_SAFE_INTEGER);
    // Check column overlap
    const colOverlap = Math.max(range1.startCol, range2.startCol) <=
 ...
 */
export function rangesOverlapOrAdjacent(range1, range2) {
    // Must be on same sheet
    if (range1.sheetName !== range2.sheetName) {
        return false;
    }
    ...
    const rowAdjacent = Math.max(range1.startRow, range2.startRow) <=
        Math.min(range1.endRow || Number.MAX_SAFE_INTEGER, range2.endRow || Number.MAX_SAFE_INTEGER) +
            1;
    // Check column adjacency (within 1 column)
    const colAdjacent = Math.max(range1.startCol, range2.startCol) <=
        Math.min(range1.endCol || Number.MAX_SAFE_INTEGER, range2.endCol || Number.MAX_SAFE_INTEGER) +
            1;
 ...
 */
export function splitResponse(mergedData, mergedRange, targetRange) {
...
//# sourceMappingURL=request-merger.js.map