...
export class ImpactAnalyzer {
    ...
    async analyzeOperation(operation) {
        ...
        // Determine severity
        const severity = this.calculateSeverity(cells, formulasAffected.length, chartsAffected.length, protectedRangesAffected.length);
        // Generate warnings
        const warnings = this.generateWarnings(cells, formulasAffected, chartsAffected, pivotTablesAffected, protectedRangesAffected);
        // Update statistics
        warnings.forEach((w) => {
        ...
        this.log(`Impact analysis complete: ${cells} cells, ${warnings.length} warnings, ${severity} severity`);
        return analysis;
    }
    /**
     ...
     */
    extractRange(params) {
        return params['range'] || params['targetRange'] || 'A1';
    }
    /**
     * Calculate range size
     */
    calculateRangeSize(range) {
     ...
     */
    formulaReferencesRange(formula, range) {
    ...
    gridRangeToA1(gridRange) {
        ...
        const endCol = this.numberToColumn((gridRange.endColumnIndex ?? startCol.charCodeAt(0) - 65) + 1);
    ...
    rangesOverlap(range1, range2) {
        ...
        return (range1.toUpperCase().includes(range2.toUpperCase()) ||
    ...
    getChartType(spec) {
        if (spec.basicChart)
            return spec.basicChart.chartType || 'BASIC';
        if (spec.pieChart)
            return 'PIE';
        if (spec.bubbleChart)
            return 'BUBBLE';
     ...
     */
    async fetchComprehensiveData(range) {
        ...
        try {
            ...
            this.log(`Error fetching comprehensive data: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }
    }
    /**
     ...
     */
    parseFormulasFromData(data, range) {
        if (!data?.sheets)
            return [];
        const affected = [];
     ...
     */
    parseChartsFromData(data, range) {
        if (!data?.sheets)
            return [];
        const affected = [];
        for (const sheet of data.sheets) {
     ...
     */
    parsePivotTablesFromData(data, _range) {
        if (!data?.sheets)
            return [];
     ...
     */
    parseValidationRulesFromData(data, _range) {
        if (!data?.sheets)
            return [];
        const affected = [];
        for (const sheet of data.sheets) {
     ...
     */
    parseNamedRangesFromData(data, range) {
        if (!data?.namedRanges)
            return [];
        const affected = [];
     ...
     */
    parseProtectedRangesFromData(data, _range) {
        if (!data?.sheets)
            return [];
        const affected = [];
        for (const sheet of data.sheets) {
     ...
     */
    async findAffectedFormulas(range) {
        if (!this.googleClient) {
        ...
        try {
            ...
            this.log(`Error finding affected formulas: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
    /**
     ...
     */
    async findAffectedCharts(range) {
        if (!this.googleClient) {
        ...
        try {
            ...
            this.log(`Error finding affected charts: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
    /**
     ...
     */
    async findAffectedPivotTables(range) {
        if (!this.googleClient) {
            return [];
        }
        try {
                ...
                for (const sheet of sheets) {
                    ...
                    for (const pivot of sheet.pivotTables) {
                        ...
                        if (this.rangesOverlap(pivotA1, params.range)) {
                            ...
                            this.log(`Pivot table ${pivot.pivotTableId} in ${sheetName} affected by range ${range}`);
                        }
                    }
                }
            }
            ...
            this.log(`Error finding pivot tables: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
    /**
     ...
     */
    async findAffectedValidationRules(range) {
        if (!this.googleClient) {
            return [];
        }
        try {
            ...
            this.log(`Error finding validation rules: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
    /**
     ...
     */
    async findAffectedNamedRanges(range) {
        if (!this.googleClient) {
            return [];
        }
        try {
            ...
            this.log(`Error finding named ranges: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
    /**
     ...
     */
    async findAffectedProtectedRanges(range) {
        if (!this.googleClient) {
            return [];
        }
        try {
            ...
            this.log(`Error finding protected ranges: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
    /**
     ...
     */
    estimateExecutionTime(operation, cellCount) {
        // Base time: 100ms
        let time = 100;
     ...
     */
    calculateSeverity(cells, formulas, charts, protectedRanges) {
     ...
     * Generate warnings
     */
    generateWarnings(cells, formulas, charts, pivotTables, protectedRanges) {
        const warnings = [];
     ...
     * Generate recommendations
     */
    generateRecommendations(operation, warnings, severity) {
...
// Singleton instance
let impactAnalyzerInstance = null;
/**
 * Initialize impact analyzer (call once during server startup)
 */
export function initImpactAnalyzer(googleClient) {
 ...
 * @internal
 */
export function resetImpactAnalyzer() {
