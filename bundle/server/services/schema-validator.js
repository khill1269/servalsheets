...
import { DiscoveryApiClient, } from './discovery-client.js';
...
export class SchemaValidator {
     ...
     */
    async validateAgainstCurrent(api) {
            ...
            if (!currentSchema) {
                if (!this.discoveryClient.isEnabled()) {
                    return {
                        ...
                        recommendation: 'Enable Discovery API with DISCOVERY_API_ENABLED=true to detect schema changes.',
                    };
                }
     ...
     */
    async compareSchemas(api, currentSchema, newSchema) {
        logger.info('Comparing schemas for compatibility', {
            api,
            currentVersion: currentSchema.version,
            newVersion: newSchema.version,
        });
        const comparison = this.discoveryClient.compareSchemas(api, newSchema.version, currentSchema, newSchema);
     ...
     */
    generateMigrationPlan(comparison) {
        const steps = [];
        let stepOrder = 1;
        ...
        const hasBreakingChanges = comparison.removedMethods.length > 0 ||
            comparison.changedFields.length > 0 ||
            comparison.deprecatedFields.length > 0;
        const totalChanges = comparison.newFields.length +
    ...
    formatMigrationReport(plan) {
        ...
        if (plan.hasBreakingChanges) {
            lines.push('⚠️ **WARNING**: This migration includes breaking changes that require code updates.');
            lines.push('');
        }
        lines.push('## Migration Steps', '');
        for (const step of plan.steps) {
            const priorityEmoji = step.priority === 'required' ? '🔴' : step.priority === 'recommended' ? '🟡' : '🟢';
     ...
     */
    detectCommonIssues(schema, issues) {
     ...
     */
    generateRecommendation(issues, comparison) {
     ...
     */
    getTestingRequirements(comparison) {
        const requirements = [];
     ...
     */
    generateDeprecationExample(field) {
...
// Use the recommended alternative field or method`;
     ...
     */
    generateTypeChangeExample(field) {
        // OK: Explicit empty - no field provided
        if (!field)
            return undefined;
        return `// Before
