...
export class TaskManager {
    ...
    registerTask(taskId, metadata) {
        ...
        const activeTasks = Array.from(this.tasks.values()).filter((task) => task.status === 'pending' || task.status === 'running');
        if (activeTasks.length >= this.options.maxTasks) {
            throw new Error(`Maximum concurrent tasks reached (${this.options.maxTasks}). ` +
     ...
     */
    updateTaskProgress(taskId, progress, progressMessage) {
