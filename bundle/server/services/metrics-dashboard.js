 ...
 */
async function getMetricValue(metricName, labels = {}) {
 ...
 */
async function getMetricsByLabel(metricName, labelName) {
    const metrics = await register.getMetricsAsJSON();
    const metric = metrics.find((m) => m.name === metricName);
    if (!metric)
        return new Map();
...
export async function generateMetricsDashboard() {
    ...
    const efficiencyGain = estimatedUnoptimizedCalls > 0
        ? `${((totalCallsSaved / estimatedUnoptimizedCalls) * 100).toFixed(1)}%`
        : '0%';
    // Calculate cache hit rate
    const totalCacheAccess = cacheHits + cacheMisses;
    const cacheHitRate = totalCacheAccess > 0 ? `${((cacheHits / totalCacheAccess) * 100).toFixed(1)}%` : '0%';
    // Calculate success rate
    const successRate = totalToolCalls > 0 ? `${((successfulCalls / totalToolCalls) * 100).toFixed(1)}%` : '0%';
    // Get top tools
    ...
    const operationsPerMinute = uptimeSeconds > 0 ? (totalToolCalls / (uptimeSeconds / 60)).toFixed(2) : '0';
 ...
 */
export function formatDashboardAsText(dashboard) {
