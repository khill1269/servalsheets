 ...
 */
const SUPPORTED_SCOPES = ['sheets:read', 'sheets:write', 'sheets:admin'];
/**
...
export class OAuthProvider {
    ...
    sessionStore;
    cleanupInterval;
    jwtSecrets; // Active JWT secrets (primary + previous)
    oauthCircuit;
    constructor(config) {
        this.config = {
        ...
        circuitBreakerRegistry.register('google-oauth', this.oauthCircuit, 'OAuth token exchange circuit breaker');
        // ✅ SECURITY: Validate production requirements
        const isProduction = process.env['NODE_ENV'] === 'production';
        if (isProduction && !config.sessionStore && !process.env['REDIS_URL']) {
            throw new Error('Redis session store required in production (REDIS_URL not set). ' +
     ...
     */
    scopeIncludes(grantedScope, requiredScope) {
     ...
     */
    async generateState(clientId, redirectUri) {
    ...
    createRouter() {
        ...
        router.get('/oauth/authorize', async (req, res) => {
            ...
            await this.sessionStore.set(`authcode:${code}`, {
                code,
                clientId: client_id,
                redirectUri: redirect_uri,
                scope: validatedScope,
        ...
        router.get('/oauth/google-callback', async (req, res) => {
            ...
            try {
                ...
                await this.sessionStore.set(`authcode:${authCode}`, {
                    code: authCode,
        ...
        router.post('/oauth/register', express.json(), async (req, res) => {
            try {
                const { redirect_uris, client_name, grant_types, response_types, scope, token_endpoint_auth_method, } = req.body;
                ...
                await this.sessionStore.set(`dcr:${clientId}`, clientData, 365 * 24 * 60 * 60 * 1000 // 1 year
                );
     ...
     */
    async handleAuthorizationCode(code, redirectUri, codeVerifier, res) {
        ...
        await this.sessionStore.set(`refresh:${refreshTokenValue}`, {
            userId,
     ...
     */
    async handleRefreshToken(refreshToken, res) {
        ...
        await this.sessionStore.set(`refresh:${newRefreshToken}`, {
            ...tokenData,
            googleRefreshToken: tokenData.googleRefreshToken,
            expiresAt: Date.now() + this.config.refreshTokenTtl * 1000,
        }, this.config.refreshTokenTtl);
     ...
     */
    getGoogleToken(req) {
        return req.auth?.googleAccessToken;
    }
    /**
     * Extract Google refresh token from validated request
     */
    getGoogleRefreshToken(req) {
        return req.auth?.googleRefreshToken;
    }
}
