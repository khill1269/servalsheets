...
export function getSchemaIndex() {
    const index = {
        ...
        description: 'Index of all available tool schemas. Read individual schema resources for full details.',
        ...
        usage: {
            instructions: 'Before calling a tool, read its schema resource to understand available actions and parameters.',
            example: 'Read schema://tools/sheets_data to see all data operations (read, write, append, etc.)',
        },
    };
 ...
 */
export async function readSchemaResource(uri) {
...
export function registerSchemaResources(server) {
    try {
        ...
        server.registerResource('Tool Schema', 'schema://tools/{toolName}', {
            description: 'Full JSON Schema for a specific tool. Includes all actions, parameters, and validation rules.',
            mimeType: 'application/json',
        }, async (uri) => readSchemaResource(typeof uri === 'string' ? uri : String(uri)));
