...
describe('MCP capability workflow integration', () => {
  ...
  it('chains completions, resources, prompts, session context, and task execution', async () => {
    ...
    const masterIndex = resources.resources.find(
      (resource) => resource.uri === 'servalsheets://index'
    );
    ...
    const welcomeText =
      welcomePrompt.messages[0]?.content.type === 'text'
  ...
  it('advertises the current discovery surface and excludes retired legacy resource URIs', async () => {
    ...
    expect(
      masterIndex.promptCatalog.buckets.some(
        (bucket) =>
          bucket.id === 'analyze' && bucket.prompts.some((prompt) => prompt.name === 'auto_analyze')
      )
    ).toBe(true);
