...
describe("Task Endpoints (SEP-1686)", () => {
  ...
  beforeAll(async () => {
    ...
    taskStore = (harness.server as unknown as { taskStore: TaskStoreAdapter })
      .taskStore;
  });
  ...
  describe("tasks/get endpoint", () => {
    ...
    it("should return error for non-existent task", async () => {
      await expect(tasksClient().getTask("task_nonexistent")).rejects.toThrow(
        /not found/i,
      );
    });
  ...
  describe("tasks/list endpoint", () => {
    it("should list all tasks", async () => {
      const task1 = await taskStore
        .getUnderlyingStore()
        .createTask({ ttl: 300000 });
      const task2 = await taskStore
        .getUnderlyingStore()
        .createTask({ ttl: 300000 });
    ...
    it("should support cursor-based pagination", async () => {
      ...
      if (page1.nextCursor) {
        ...
        const page1Ids = new Set(
          (page1 as TaskListResponse).tasks.map((t) => t.taskId),
        );
  ...
  describe("tasks/cancel endpoint", () => {
    ...
    it("should reject cancellation of terminal task", async () => {
      ...
      await expect(tasksClient().cancelTask(task.taskId)).rejects.toThrow(
    ...
    it("should reject cancellation of non-existent task", async () => {
      await expect(
        tasksClient().cancelTask("task_nonexistent"),
  ...
  describe("tasks/result endpoint (blocking)", () => {
    it("should wait for task completion and return result", async () => {
      ...
      const resultPromise = tasksClient().getTaskResult(
        task.taskId,
        taskResultSchema,
      );
    ...
    it("should return immediately if task already completed", async () => {
      ...
      const result = await tasksClient().getTaskResult(
        task.taskId,
        taskResultSchema,
      );
  ...
  describe("Task expiration", () => {
    it("should expire task after TTL", async () => {
      ...
      await expect(tasksClient().getTask(task.taskId)).rejects.toThrow(
        /not found/i,
      );
    });
  });
