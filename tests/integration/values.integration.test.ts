...
describe.skipIf(SKIP_INTEGRATION)("Values Handler Integration", () => {
  ...
  const callValues = (request: Record<string, unknown>) =>
    callTool<{ response: Record<string, unknown> }>("sheets_values", request);

  const callSheets = (request: Record<string, unknown>) =>
    callTool<{ response: Record<string, unknown> }>("sheets_sheet", request);

  const callVersions = (request: Record<string, unknown>) =>
    callTool<{ response: Record<string, unknown> }>("sheets_versions", request);

  const expectSuccess = <T extends Record<string, unknown>>(payload: {
    response: T;
  }): T => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(
      keyPath,
      JSON.stringify(credentials.serviceAccount, null, 2),
    );
  ...
  describe("Clear Operations", () => {
    ...
    it("should respect effect scope limits", async () => {
      ...
      expect(
        (response.response as { error?: { code?: string } }).error?.code,
      ).toBe("EFFECT_SCOPE_EXCEEDED");
    });
  });
  ...
  describe("Find and Replace", () => {
    ...
    it("should support matchEntireCell for exact matches", async () => {
      ...
      const payload = expectSuccess(response);
      expect(
        (payload["matches"] as Array<{ value: string }>).map((m) => m.value),
      ).toEqual(["foo"]);
    });
  });
  ...
  describe("Batch Operations", () => {
    ...
    it("should batch write to multiple ranges", async () => {
      ...
      const readPayload = expectSuccess(readBack);
      expect((readPayload["values"] as unknown[][])[0]).toEqual([
        "Batch1",
        "Batch3",
      ]);
    });
  ...
  describe("Error Handling", () => {
    ...
    permissionTest(
      "should return proper error for permission denied",
      async () => {
        ...
        expect(
          (response.response as { error?: { code?: string } }).error?.code,
        ).toMatch(/PERMISSION_DENIED|NOT_FOUND/);
      },
    );
  });
  ...
  describe("Safety Rails", () => {
    ...
    it("should create auto-snapshot for destructive operations", async () => {
      ...
      const payload = expectSuccess(response);
      const snapshotId = (
        payload["mutation"] as { revertSnapshotId?: string } | undefined
      )?.revertSnapshotId;
